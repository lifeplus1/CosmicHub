...
console.log(
  `Sun at ${sunPosition}° should be in:`,
  getSignFromDegree(sunPosition)
);
console.log(
  `Expected: Capricorn (273° / 30 = ${Math.floor(sunPosition / 30)} = index 9 = Capricorn)`
);

// Test with Moon position
const moonPosition = 76.89457911765973;
console.log(
  `Moon at ${moonPosition}° should be in:`,
  getSignFromDegree(moonPosition)
);
console.log(
  `Expected: Gemini (76° / 30 = ${Math.floor(moonPosition / 30)} = index 2 = Gemini)`
);
...
const calculatePlanetHouse = (planetPosition, houses) => {
  ...
  console.log(
    `Calculating house for planet at ${planetPosition}° (normalized: ${normalizedPosition}°)`
  );
  ...
  for (let i = 0; i < houses.length; i++) {
    ...
    console.log(
      `Checking house ${currentHouse.house}: cusp at ${currentCusp}°, next cusp at ${nextCusp}°`
    );
...
const sunHouse = calculatePlanetHouse(
  sunPosition,
  savedChartData.chart_data.houses
);
console.log(`Sun house: ${sunHouse}`);

// Test Moon house calculation
console.log('\n--- Testing Moon House Calculation ---');
const moonHouse = calculatePlanetHouse(
  moonPosition,
  savedChartData.chart_data.houses
);
