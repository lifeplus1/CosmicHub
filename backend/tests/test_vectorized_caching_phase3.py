...
class TestPersistentCache:
    ...
    def setup_method(self):
        ...
        self.cache = PersistentCache(
            self.temp_dir, max_age_hours=0.1
...
class TestTieredCacheManager:
    ...
    def test_memory_cache_priority(self):
        ...
        self.cache_manager.put(
            "test_value", "aspect_calculation", chart_data=chart_data, orb=8.0
        )

        # Should hit memory cache
        result = self.cache_manager.get(
            "aspect_calculation", chart_data=chart_data, orb=8.0
        )
        assert result == "test_value"
    ...
    def test_persistent_cache_fallback(self):
        ...
        if self.cache_manager.persistent_cache:
            key = self.cache_manager._create_cache_key(
                "aspect_calculation", chart_data=chart_data, orb=8.0
            )
        ...
        result2 = self.cache_manager.get(
            "aspect_calculation", chart_data=chart_data, orb=8.0
        )
    ...
    def test_cache_key_generation(self):
        ...
        key1 = self.cache_manager._create_cache_key(
            "test_calc", chart_data=chart1, orb=8.0
        )
        key2 = self.cache_manager._create_cache_key(
            "test_calc", chart_data=chart2, orb=8.0
        )
    ...
    def test_different_parameters_different_keys(self):
        ...
        key1 = self.cache_manager._create_cache_key(
            "test_calc", chart_data=chart_data, orb=8.0
        )
        key2 = self.cache_manager._create_cache_key(
            "test_calc", chart_data=chart_data, orb=10.0
        )
