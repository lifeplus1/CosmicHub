...
from utils.vectorized_memory_optimization import (
    ArrayMemoryPool, MemoryOptimizedVectorizedCalculator, 
    memory_optimized_processing, MemoryMonitor, get_global_memory_pool
)
...
class TestArrayMemoryPool:
    ...
    def test_memory_pool_stats(self):
        ...
        assert final_stats['arrays_allocated'] >= initial_stats['arrays_allocated']
...
class TestMemoryOptimizedVectorizedCalculator:
    ...
    def test_large_batch_processing_simulation(self):
        ...
        for i in range(10):  # Small batch for testing
            chart1 = {f'Planet{j}': float((i * 30 + j * 40) % 360) for j in range(10)}
            chart2 = {f'Planet{j}': float((i * 35 + j * 45) % 360) for j in range(10)}
    ...
    def test_chunked_processing_memory_efficiency(self):
        ...
        for i in range(20):  # Create 20 charts each
            chart1 = {f'Planet{j}': float((i * 25 + j * 15) % 360) for j in range(10)}
            chart2 = {f'Planet{j}': float((i * 30 + j * 20) % 360) for j in range(10)}
            charts1.append(chart1)
            charts2.append(chart2)
        
        # Test memory estimates
        estimates = self.calculator.estimate_memory_usage(
            num_charts1=len(charts1),
            num_charts2=len(charts2)
        )
...
class TestMemoryMonitor:
    ...
    def test_memory_monitoring(self):
        ...
        if final_stats['current_memory_mb'] > 0:  # Only if psutil is available
            assert final_stats['peak_memory_mb'] >= final_stats['start_memory_mb']
...
class TestMemoryOptimizedContextManager:
    ...
    def test_context_manager_basic(self):
        ...
        with memory_optimized_processing(chunk_size=10, enable_pooling=True) as (calculator, monitor):
...
class TestIntegrationWithVectorizedCalculations:
    ...
    def test_memory_optimized_aspect_calculations(self):
        ...
        with memory_optimized_processing(chunk_size=5) as (calculator, monitor):
