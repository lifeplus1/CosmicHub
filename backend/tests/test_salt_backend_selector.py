...
from utils.salt_backend import get_salt_backend, InMemorySaltBackend


def test_default_backend_memory(monkeypatch):  # type: ignore[no-untyped-def]
    monkeypatch.delenv("SALT_BACKEND", raising=False)  # type: ignore[attr-defined]
    b = get_salt_backend(refresh=True)
    assert isinstance(b, InMemorySaltBackend)


def test_firestore_flag_falls_back(monkeypatch):  # type: ignore[no-untyped-def]
