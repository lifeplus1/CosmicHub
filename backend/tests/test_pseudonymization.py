...
class TestSaltStorage:
    ...
    def test_custom_salt_intervals(self):
        ...
        with patch.dict('os.environ', {'USER_SALT_ROTATION_DAYS': '60', 'GLOBAL_SALT_ROTATION_DAYS': '15'}):
...
def test_full_pseudonymization_workflow():
    ...
    sensitive_data: List[str] = ["user_email@example.com", "192.168.1.1", "session_token_123"]
    ...
    new_pseudonyms: List[str] = [pseudonymize(data, new_salt) for data in sensitive_data]
    ...
    new_pseudonyms_2: List[str] = [pseudonymize(data, new_salt) for data in sensitive_data]
