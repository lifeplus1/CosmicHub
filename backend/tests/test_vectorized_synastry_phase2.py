...
from utils.vectorized_aspect_utils import VectorizedAspectCalculator, build_aspect_matrix_fast, batch_synastry_analysis
from utils.aspect_utils import build_aspect_matrix, get_key_aspects, PLANETS
...
class TestVectorizedAspectCalculatorCore:
    ...
    def setup_method(self):
        ...
        self.edge_chart = {
            'sun': 359.0, 'moon': 1.0, 'mercury': 179.0, 'venus': 181.0, 'mars': 89.0,
            'jupiter': 91.0, 'saturn': 269.0, 'uranus': 271.0, 'neptune': 0.0, 'pluto': 180.0
        }

    def test_separation_matrix_calculation(self):
        """Test basic separation matrix calculation."""
        result = self.calculator.calculate_separation_matrix(self.chart1, self.chart2)
        ...
        for i in range(10):
            assert abs(result[i, i] - 15.0) < 0.001, f"Planet {i} separation incorrect: {result[i, i]}"

    def test_separation_matrix_wraparound(self):
        """Test zodiacal wraparound handling."""
        # Create charts with wraparound scenarios
        chart_a = {'sun': 350.0, 'moon': 10.0, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0, 
                   'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        chart_b = {'sun': 10.0, 'moon': 350.0, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0, 
                   'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        
        result = self.calculator.calculate_separation_matrix(chart_a, chart_b)
        
        # Both should show 20° separation (not 340°)
        assert abs(result[0, 0] - 20.0) < 0.001, f"Sun wraparound incorrect: {result[0, 0]}"
        assert abs(result[1, 1] - 20.0) < 0.001, f"Moon wraparound incorrect: {result[1, 1]}"
    ...
    def test_vectorized_aspect_detection(self):
        ...
        result = self.calculator.build_aspect_matrix_vectorized(chart1_test, chart2_test)
        ...
        assert aspects_found > 0, "No aspects detected in vectorized calculation"
    ...
    def test_accuracy_against_traditional(self):
        ...
        vectorized_result = self.calculator.build_aspect_matrix_vectorized(self.chart1, self.chart2)
        ...
        for i in range(10):
            for j in range(10):
                ...
                if trad_cell is None and vect_cell is None:
                    ...
                    pytest.fail(f"Mismatch at [{i}][{j}]: traditional={trad_cell}, vectorized={vect_cell}")
                else:
                    # Both have aspects - compare them
                    assert trad_cell['aspect'] == vect_cell['aspect'], \
                        f"Aspect mismatch at [{i}][{j}]: {trad_cell['aspect']} vs {vect_cell['aspect']}"
                    
                    # Orbs should be very close (within 0.1°)
                    assert abs(trad_cell['orb'] - vect_cell['orb']) < 0.1, \
    ...
    def test_performance_improvement(self):
        ...
        large_chart1 = {planet: float(i * 36 + (i * 7) % 360) for i, planet in enumerate(PLANETS)}
        large_chart2 = {planet: float(i * 43 + (i * 11) % 360) for i, planet in enumerate(PLANETS)}
        
        # Warm up
        self.calculator.build_aspect_matrix_vectorized(large_chart1, large_chart2)
        ...
        assert improvement_ratio >= 0.8, f"Vectorized slower than expected: {improvement_ratio:.2f}x"
...
class TestVectorizedIntegration:
    ...
    def setup_method(self):
        self.chart1 = {
            'sun': 120.0, 'moon': 240.0, 'mercury': 90.0, 'venus': 60.0, 'mars': 180.0,
            'jupiter': 0.0, 'saturn': 150.0, 'uranus': 330.0, 'neptune': 210.0, 'pluto': 30.0
        }
        self.chart2 = {
            'sun': 300.0, 'moon': 60.0, 'mercury': 270.0, 'venus': 240.0, 'mars': 0.0,
            'jupiter': 180.0, 'saturn': 330.0, 'uranus': 150.0, 'neptune': 30.0, 'pluto': 210.0
        }
    ...
    def test_error_handling(self):
        ...
        # Should not crash
        result = calculator.build_aspect_matrix_vectorized(incomplete_chart1, incomplete_chart2)
        assert len(result) == 10  # Still 10x10 matrix
        
        # Test with empty charts
        empty_chart: Dict[str, float] = {}
        result = calculator.build_aspect_matrix_vectorized(empty_chart, empty_chart)
        assert len(result) == 10

    def test_data_type_consistency(self):
        """Test that results have consistent data types."""
        calculator = VectorizedAspectCalculator()
        result = calculator.build_aspect_matrix_vectorized(self.chart1, self.chart2)
        ...
        for row in result:
            for cell in row:
                if cell is not None:
                    ...
                    assert cell['orb'] >= 0.0
                    assert cell['type'] in ['harmonious', 'challenging', 'neutral']
...
class TestVectorizedEdgeCases:
    ...
    def test_exact_aspects(self):
        ...
        chart1 = {'sun': 0.0, 'moon': 60.0, 'mercury': 90.0, 'venus': 120.0, 'mars': 180.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        chart2 = {'sun': 0.0, 'moon': 0.0, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        ...
        assert result[0][0] is not None and result[0][0]['aspect'] == 'conjunction'  # Sun-Sun 0°
        assert result[1][0] is not None and result[1][0]['aspect'] == 'sextile'      # Moon-Sun 60°
        assert result[2][0] is not None and result[2][0]['aspect'] == 'square'       # Mercury-Sun 90°
        assert result[3][0] is not None and result[3][0]['aspect'] == 'trine'        # Venus-Sun 120°
        assert result[4][0] is not None and result[4][0]['aspect'] == 'opposition'   # Mars-Sun 180°
    ...
    def test_orb_boundaries(self):
        ...
        chart1 = {'sun': 0.0, 'moon': 9.0, 'mercury': 11.0, 'venus': 0.0, 'mars': 0.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        chart2 = {'sun': 0.0, 'moon': 0.0, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        ...
        assert result[1][0] is not None and result[1][0]['aspect'] == 'conjunction'
    ...
    def test_floating_point_precision(self):
        ...
        chart1 = {'sun': 0.123456789, 'moon': 60.987654321, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
        chart2 = {'sun': 120.111111111, 'moon': 240.999999999, 'mercury': 0.0, 'venus': 0.0, 'mars': 0.0,
                  'jupiter': 0.0, 'saturn': 0.0, 'uranus': 0.0, 'neptune': 0.0, 'pluto': 0.0}
