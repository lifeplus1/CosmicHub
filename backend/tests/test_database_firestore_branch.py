...
class FakeChartsCollection:
    ...
    def stream(self):  # noqa: D401
        ...
        docs.sort(
            key=lambda d: d.to_dict().get("created_at", ""), reverse=True
        )
...
class FakeUserDoc:
    ...
    def set(
        self, data: Mapping[str, Any], merge: bool = False
    ) -> None:  # noqa: D401
        if merge:
            self._data.update(data)
        else:
...
class FakeDB:
    ...
    def batch(self):  # noqa: D401
        class B:
            ...
            def set(
                self, doc_ref: FakeChartDoc, data: Mapping[str, Any]
            ) -> None:  # noqa: D401
                doc_ref.set(data)
