...
def test_interpretation_metrics_exposed(monkeypatch: MonkeyPatch):
    ...
    save_resp = client.post(
        "/api/charts/save",
        json={
            "planets": [{"name": "Sun", "sign": "Leo", "degree": 10.5, "house": 5, "aspects": []}],
            "asteroids": [],
    ...
    interp_resp = client.post(
        ...
        json={"chartId": chart_id, "userId": "dev-user", "type": "natal", "interpretation_level": "advanced"},
        headers={"Authorization": "Bearer test"}
    )
    ...
    assert 'interpretations_total' in body or 'interpretation_generation_seconds' in body
