...
from utils.vectorized_monitoring import VectorizedPerformanceMonitor, PerformanceMetrics, get_performance_monitor
...
class TestVectorizedPerformanceMonitor:
    ...
    def setup_method(self):
        ...
        self.monitor = VectorizedPerformanceMonitor(enable_detailed_logging=False)
    ...
    def test_operation_timing(self):
        ...
        with self.monitor.time_operation("test_operation", num_calculations=3) as context:
    ...
    def test_memory_usage_tracking(self):
        ...
        with self.monitor.time_operation("memory_test", num_calculations=1) as context:
    ...
    def test_baseline_establishment(self):
        ...
        for i in range(5):
            with self.monitor.time_operation("baseline_test", num_calculations=10):
    ...
    def test_performance_regression_detection(self):
        ...
        for i in range(5):
            with self.monitor.time_operation("regression_test", num_calculations=1):
                time.sleep(0.001)
        ...
        regression = self.monitor.detect_performance_regression("regression_test", threshold_factor=2.0)
    ...
    def test_metrics_retention(self):
        ...
        assert len(short_monitor.metrics_history) == 1
        assert short_monitor.metrics_history[0].operation_name == "retention_test2"
...
class TestVectorizedAspectCalculatorWithMonitoring:
    ...
    def test_monitored_aspect_calculation(self):
        ...
        chart1 = {
            'Sun': 0.0, 'Moon': 60.0, 'Mercury': 90.0, 'Venus': 120.0, 'Mars': 180.0,
            'Jupiter': 210.0, 'Saturn': 240.0, 'Uranus': 270.0, 'Neptune': 300.0, 'Pluto': 330.0
        }
        chart2 = {
            'Sun': 5.0, 'Moon': 58.0, 'Mercury': 95.0, 'Venus': 117.0, 'Mars': 175.0,
            'Jupiter': 30.0, 'Saturn': 120.0, 'Uranus': 0.0, 'Neptune': 240.0, 'Pluto': 90.0
        }
        
        with self.monitor.time_operation("vectorized_aspect_calculation", num_calculations=100) as context:
            # Calculate separation matrix
            separations = self.calculator.calculate_separation_matrix(chart1, chart2)
            context.add_array_info("separations", separations)
            
            # Build aspect matrix
            aspects = self.calculator.build_aspect_matrix_vectorized(chart1, chart2)
    ...
    def test_batch_processing_monitoring(self):
        ...
        chart_pairs = []
        for i in range(5):
            chart1 = {planet: float(i * 10 + j * 30) % 360 for j, planet in enumerate(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto'])}
            chart2 = {planet: float((i + 1) * 15 + j * 25) % 360 for j, planet in enumerate(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto'])}
            chart_pairs.append((chart1, chart2))
        
        with self.monitor.time_operation("batch_aspect_calculation", num_calculations=len(chart_pairs)) as context:
            results = []
            for chart1, chart2 in chart_pairs:
                result = self.calculator.build_aspect_matrix_vectorized(chart1, chart2)
                results.append(result)
            
            context.add_metric("total_chart_pairs", len(chart_pairs))
            context.add_metric("avg_aspects_per_pair", sum(np.count_nonzero(r) for r in results) / len(results))
    ...
    def test_performance_comparison_monitoring(self):
        ...
        with self.monitor.time_operation("vectorized_method", num_calculations=1):
            vectorized_result = self.calculator.build_aspect_matrix_vectorized(chart1, chart2)
        
        # Monitor traditional method (if available)
        try:
            from utils.aspect_utils import build_aspect_matrix, PLANETS
            
            with self.monitor.time_operation("traditional_method", num_calculations=1):
                traditional_result = build_aspect_matrix(chart1, chart2, PLANETS[:len(chart1)])
