...
async def test_ai_service_methods():
    ...
    section_result = await svc.generate_section_interpretation(
        chart, "overview", "user123"
    )
...
def test_stripe_service_session(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_create(**kwargs: Any):
        ...
        return types.SimpleNamespace(
            id="sess_test", url="https://stripe.test/sess_test"
        )
    ...
    result = stripe_service.create_stripe_session(
        "user@example.com", "price_123"
    )
