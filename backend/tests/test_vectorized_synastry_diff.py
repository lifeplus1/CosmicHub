...
from backend.utils.aspect_utils import build_aspect_matrix, PLANETS
    ...
    for r_trad, r_vec in zip(trad, vec):
        ...
        for cell_a, cell_b in zip(r_trad, r_vec):
            if cell_a is None or cell_b is None:
                assert cell_a is cell_b, "Mismatch in presence/absence of aspect cell"
            else:
        ...
        for r_trad, r_vec in zip(trad, vec):
            for cell_a, cell_b in zip(r_trad, r_vec):
                if cell_a is not None:
                    freq_trad[cell_a["aspect"]] = freq_trad.get(cell_a["aspect"], 0) + 1
                if cell_b is not None:
                    freq_vec[cell_b["aspect"]] = freq_vec.get(cell_b["aspect"], 0) + 1
