...
class ConnectionManager:
    ...
    async def send_streaming_chunk(
        self, session_id: str, chunk: str, chunk_type: str = "text"
    ):
    ...
    async def send_error(self, session_id: str, error: str):
        """Send error message"""
        message: Dict[str, Any] = {"type": "error", "data": {"error": error}}
        await self.send_message(session_id, message)

    async def send_completion(self, session_id: str):
        """Send completion signal"""
        message: Dict[str, Any] = {"type": "complete", "data": {}}
        await self.send_message(session_id, message)
...
async def websocket_ai_interpret(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.accept()
        logger.info(
            f"WebSocket connection established: session_id={session_id}"
        )
            ...
            try:
                ...
                if message_type == "interpret":
                    ...
                    try:
                        ...
                        user_id = (
                            "elite_user_dev_123"  # Mock user for development
                        )
                        ...
                        await process_interpretation_request(
...
async def process_interpretation_request(
    ...
    try:
        ...
        interpretation_type = message_data.get(
            "interpretation_type", "advanced"
        )
        ...
        async for chunk in generate_interpretation_stream(
            chart_data, interpretation_type
        ):
        ...
        await manager.send_error(
            session_id, f"Interpretation failed: {str(e)}"
        )


async def generate_interpretation_stream(
    chart_data: Dict[str, Any], interpretation_type: str
):
    """Generate interpretation with streaming text chunks"""
    try:
        # Get the full interpretation first
        interpretation_result = generate_interpretation(
            chart_data, interpretation_type
        )
        ...
        if isinstance(interpretation_result, dict):
            ...
            sections = [
                (
                    "Core Identity",
                    interpretation_result.get("core_identity", {}),
                ),
                (
                    "Life Purpose",
                    interpretation_result.get("life_purpose", {}),
                ),
                (
                    "Relationship Patterns",
                    interpretation_result.get("relationship_patterns", {}),
                ),
                ("Career Path", interpretation_result.get("career_path", {})),
                (
                    "Growth Challenges",
                    interpretation_result.get("growth_challenges", {}),
                ),
                (
                    "Spiritual Gifts",
                    interpretation_result.get("spiritual_gifts", {}),
                ),
            ]
