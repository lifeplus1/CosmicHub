...
async def save_user_chart(
    request: SaveChartRequest, user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    try:
        ...
        logger.info(
            f"Chart saved successfully for user {user_id}: {saved_chart['id']}"
        )
...
async def test_save_user_chart(request: SaveChartRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save test chart: {str(e)}"
        )
...
async def get_user_charts(
    ...
    try:
        charts = get_charts(
            user_id=user_id, limit=limit, start_after=start_after
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve charts: {str(e)}"
        )


@router.delete("/{chart_id}")
async def delete_user_chart(
    chart_id: str, user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete chart: {str(e)}"
        )
