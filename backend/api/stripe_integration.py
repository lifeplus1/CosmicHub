...
SUBSCRIPTION_PLANS: Dict[str, SubscriptionPlanData] = {
    ...
    "astro_premium": {
        ...
        "features": ["unlimited_charts", "synastry_analysis", "transit_tracking"],
...
async def create_stripe_checkout_session(user_id: str, plan_id: str, success_url: str, cancel_url: str) -> Dict[str, Any]:
...
async def get_user_subscription_status(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        subscription_doc = db_client.collection("subscriptions").document(user_id).get()
        ...
        if subscription_data.get("stripe_subscription_id"):
            try:
                ...
                logger.warning(f"Stripe verification failed for user {user_id}: {str(e)}")
        
        return {
            "status": "active" if subscription_data.get("is_active", False) else "inactive",
            "tier": subscription_data.get("plan_id", "free"),
            "features": SUBSCRIPTION_PLANS.get(subscription_data.get("plan_id", ""), {}).get("features", []),
...
async def handle_subscription_updated(subscription: Dict[str, Any], db_client: Any) -> None:
    ...
    try:
        ...
        logger.info(f"Subscription updated for user {firebase_uid}: {subscription['status']}")
