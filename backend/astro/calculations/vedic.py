...
def calculate_vedic_planets(julian_day: float) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"ayanamsa": ayanamsa, "planets": vedic_positions}
...
def calculate_vedic_houses(
    ...
    try:
        ...
        houses: Tuple[List[float], List[float]] = houses_ex(
            julian_day, lat, lon, b"P"
        )  # Placidus system
        ...
        return {"houses": vedic_houses, "angles": vedic_angles}
...
def get_vedic_chart_analysis(vedic_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        moon_sign = (
            moon_data.get("vedic_sign", "").split()[-1]
            if moon_data
            else "Unknown"
        )
        ...
        # Find moon nakshatra
        moon_nakshatra = (
            moon_data.get("nakshatra", {}).get("name", "Unknown")
            if moon_data
            else "Unknown"
        )
        ...
        if houses and "angles" in houses:
            ...
            asc_sign = (
                VEDIC_SIGNS[sign_index]
                if sign_index < len(VEDIC_SIGNS)
                else "Unknown"
            )
