...
def calculate_gene_keys_profile(year: int, month: int, day: int, hour: int, minute: int, 
                               lat: float, lon: float, timezone: str = "UTC") -> Dict[str, Any]:
    """
    Calculate complete Gene Keys profile including all sequences
    """
    try:
        logger.info(f"Calculating Gene Keys profile for {year}-{month}-{day} {hour}:{minute}")
        
        # First get the Human Design data (Gene Keys uses same calculations)
        hd_data = calculate_human_design(year, month, day, hour, minute, lat, lon, timezone)
...
def calculate_pearl_sequence(conscious: Dict[str, Any], unconscious: Dict[str, Any]) -> Dict[str, Any]:
...
def calculate_golden_path(activation_sequence: Dict[str, Any]) -> Dict[str, Any]:
    """Calculate the Golden Path - the journey of transformation"""
    try:
        if "error" in activation_sequence:
            return {"error": "Cannot calculate Golden Path without valid Activation Sequence"}
