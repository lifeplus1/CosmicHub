...
def get_composite_chart(chart1: Dict[str, Any], chart2: Dict[str, Any]) -> Dict[str, Any]:
...
def analyze_interaspects(chart1: Dict[str, Any], chart2: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            for planet2, data2 in planets2.items():
                ...
                for aspect_angle, aspect_name, orb in aspects:
                    if abs(angle - aspect_angle) <= orb:
                        ...
                        # Get interpretation
                        interpretation = get_synastry_interpretation(planet1, planet2, aspect_name)
                        ...
                        interaspects.append({
                            ...
                            "aspect": aspect_name,
                            "orb": orb_exact,
                            "strength": "Strong" if orb_exact <= 3 else "Moderate" if orb_exact <= 6 else "Weak",
...
def get_synastry_interpretation(planet1: str, planet2: str, aspect: str) -> str:
    ...
    interpretations = {
        # Sun aspects
        ("sun", "sun", "Conjunction"): "Strong ego connection, similar life purposes, potential for mutual recognition",
        ("sun", "moon", "Conjunction"): "Deep emotional understanding, man represents woman's ideal, powerful attraction",
        ("sun", "moon", "Trine"): "Natural harmony between masculine and feminine energies, supportive relationship",
        ("sun", "moon", "Square"): "Tension between ego and emotions, challenging but potentially transformative",
        ("sun", "venus", "Conjunction"): "Strong romantic attraction, appreciation for each other's creativity",
        ("sun", "mars", "Conjunction"): "Intense physical attraction, potential for conflict or dynamic action together",
        
        # Moon aspects
        ("moon", "moon", "Conjunction"): "Deep emotional understanding, similar emotional needs and responses",
        ("moon", "venus", "Trine"): "Natural affection, emotional harmony, nurturing love",
        ("moon", "mars", "Square"): "Emotional volatility, passionate but potentially explosive dynamics",
        
        # Venus aspects
        ("venus", "venus", "Conjunction"): "Shared values in love and beauty, similar romantic expressions",
        ("venus", "mars", "Conjunction"): "Classic romantic and sexual attraction, complementary energies",
        ("venus", "mars", "Square"): "Intense sexual chemistry with potential for relationship drama",
        
        # Mars aspects
        ("mars", "mars", "Conjunction"): "Shared drive and energy, potential for both cooperation and competition",
        
        # Mercury aspects
        ("mercury", "mercury", "Conjunction"): "Mental compatibility, easy communication and understanding",
        ("sun", "mercury", "Conjunction"): "Good intellectual connection, stimulating conversations",
        
        # Jupiter aspects
        ("sun", "jupiter", "Trine"): "Mutual growth, optimism, beneficial influence on each other",
        ("moon", "jupiter", "Trine"): "Emotional expansion, generosity, protective feelings",
        
        # Saturn aspects
        ("sun", "saturn", "Conjunction"): "Serious relationship potential, maturity, possible restrictions",
        ("moon", "saturn", "Square"): "Emotional limitation, need for patience and commitment",
        
        # Outer planet aspects
        ("sun", "uranus", "Conjunction"): "Exciting, unpredictable connection, catalyst for change",
        ("venus", "neptune", "Trine"): "Romantic idealization, spiritual love, artistic inspiration",
        ("mars", "pluto", "Square"): "Intense power struggles, transformative but potentially destructive"
    }
    ...
    return generic_aspects.get(aspect, f"{planet1.title()} {aspect.lower()} {planet2.title()} - significant connection")

def analyze_compatibility_score(interaspects: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Calculate overall compatibility score and breakdown"""
    try:
        if not interaspects:
            return {"overall_score": 0, "breakdown": {}, "interpretation": "No significant aspects found"}
        ...
        for aspect in interaspects:
            ...
            # Adjust by strength
            strength_multiplier = {"Strong": 1.0, "Moderate": 0.7, "Weak": 0.4}.get(strength, 0.5)
            ...
            if any(p in ["jupiter", "saturn", "uranus", "neptune", "pluto"] for p in planets):
        ...
        if max_possible > 0:
            for category in scores:
                scores[category] = max(0, min(100, (scores[category] / max_possible) * 100 + 50))
        ...
        if overall_score >= 80:
            ...
            interpretation = "Very good compatibility - many harmonious aspects"
        elif overall_score >= 60:
            interpretation = "Good compatibility - solid foundation with some challenges"
        elif overall_score >= 50:
            interpretation = "Moderate compatibility - requires work and understanding"
        else:
        ...
        return {"overall_score": 0, "breakdown": {}, "interpretation": "Error calculating compatibility"}

def calculate_synastry_chart(person1_data: Dict[str, Any], person2_data: Dict[str, Any]) -> Dict[str, Any]:
    """Calculate complete synastry analysis between two birth charts"""
    try:
        # Calculate individual charts
        chart1 = calculate_chart(
            person1_data["year"], person1_data["month"], person1_data["day"],
            person1_data["hour"], person1_data["minute"], 
            person1_data.get("lat"), person1_data.get("lon"),
            person1_data.get("timezone"), person1_data.get("city")
        )
        
        chart2 = calculate_chart(
            person2_data["year"], person2_data["month"], person2_data["day"],
            person2_data["hour"], person2_data["minute"],
            person2_data.get("lat"), person2_data.get("lon"),
            person2_data.get("timezone"), person2_data.get("city")
        )
        ...
        return {
            ...
            "summary": {
                ...
                "key_themes": generate_relationship_themes(interaspects, compatibility)
            }
        }
...
def analyze_house_overlays(chart1: Dict[str, Any], chart2: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for planet, data in planets1.items():
            ...
            # Get interpretation
            interpretation = get_house_overlay_interpretation(planet, house_number)
...
def get_house_overlay_interpretation(planet: str, house: int) -> str:
    ...
    interpretations = {
        ...
        ("venus", 2): "You value their resources and bring beauty to their possessions",
    ...
    return house_meanings.get(house, f"{planet.title()} has an influence in their {house}th house")

def generate_relationship_themes(interaspects: List[Dict[str, Any]], compatibility: Dict[str, Any]) -> List[str]:
    ...
    if strong_aspects:
        ...
        romantic_aspects = [a for a in strong_aspects 
                          if a["person1_planet"] in romantic_planets or a["person2_planet"] in romantic_planets]
        
        if romantic_aspects:
            themes.append("Strong romantic and emotional connection")
        
        # Look for mental compatibility
        mercury_aspects = [a for a in strong_aspects 
                          if "mercury" in [a["person1_planet"], a["person2_planet"]]]
        if mercury_aspects:
            themes.append("Excellent mental and communication compatibility")
        
        # Look for challenges
        challenging_aspects = [a for a in interaspects if a["aspect"] in ["Square", "Opposition"]]
        if len(challenging_aspects) > len(strong_aspects):
            themes.append("Relationship requires work and mutual understanding")
...
def get_element_compatibility(chart1: Dict[str, Any], chart2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for chart, element_count in [(chart1, chart1_elements), (chart2, chart2_elements)]:
        ...
        for element in elements:
            ...
            opposite = {"Fire": "Air", "Air": "Fire", "Earth": "Water", "Water": "Earth"}
            if chart1_elements[element] > 0 and chart2_elements[element] > 0:
                total_harmony += min(chart1_elements[element], chart2_elements[element]) * 2
            elif chart1_elements[element] > 0 and chart2_elements.get(opposite.get(element, ""), 0) > 0:
                total_harmony += min(chart1_elements[element], chart2_elements[opposite[element]]) * 1
