...
def get_elemental_integration(element_count: Dict[str, int]) -> str:  # type: ignore
    ...
    return (
        f"Balance dominant {dominant} element; develop {', '.join(missing)}"
        if missing
        else f"Balanced with {dominant} emphasis"
    )
...
def get_resolution_strategies(aspects: List[Dict[str, Any]]) -> str:  # type: ignore
    return (
        ...
        if aspects
        else "Maintain current growth path"
    )
...
def synthesize_integration_theme(element_count: Dict[str, int], quality_count: Dict[str, int], planet_aspect_count: Dict[str, int]) -> str:  # type: ignore
    ...
    dominant_quality = (
        max(quality_count, key=lambda k: quality_count[k])
        if quality_count
        else "none"
    )
    focal = (
        max(planet_aspect_count, key=lambda k: planet_aspect_count[k])
        if planet_aspect_count
        else "none"
    )
    return (
...
HOUSE_THEMES = {
    1: {
        "theme": "Identity & Self",
        "life_area": "personality, appearance, first impressions",
    },
    2: {
        "theme": "Values & Resources",
        "life_area": "money, possessions, self-worth",
    },
    3: {
        "theme": "Communication & Learning",
        "life_area": "siblings, local environment, education",
    },
    4: {
        "theme": "Home & Family",
        "life_area": "family, roots, emotional foundation",
    },
    5: {
        "theme": "Creativity & Romance",
        "life_area": "children, creativity, love affairs",
    },
    6: {
        "theme": "Service & Health",
        "life_area": "work, health, daily routines",
    },
    7: {
        "theme": "Partnerships",
        "life_area": "marriage, business partnerships, open enemies",
    },
    8: {
        "theme": "Transformation",
        "life_area": "shared resources, sexuality, death/rebirth",
    },
    9: {
        "theme": "Higher Wisdom",
        "life_area": "philosophy, higher education, foreign travel",
    },
    10: {
        "theme": "Career & Reputation",
        "life_area": "career, public image, authority",
    },
    11: {
        "theme": "Hopes & Groups",
        "life_area": "friends, groups, hopes and wishes",
    },
    12: {
        "theme": "Spirituality & Release",
        "life_area": "spirituality, hidden enemies, sacrifice",
    },
}
...
def generate_interpretation(
    ...
    try:
        ...
        logger.error(
            f"Error generating {interpretation_type} interpretation: {str(e)}"
        )
        return {"error": str(e)}
...
def analyze_life_purpose(chart_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        houses = chart_data.get("houses", [])
        mc_sign = (
            get_sign_from_position(houses[9]["cusp"])
            if len(houses) >= 10
            else None
        )
...
def analyze_relationship_patterns(
    ...
    try:
        ...
        partnership_sign = (
            get_sign_from_position(houses[6]["cusp"])
            if len(houses) >= 7
            else None
        )
        ...
        relationship_analysis = {  # type: ignore
            ...
            "partnership_karma": {
                ...
                "partner_qualities": (
                    get_partner_qualities(partnership_sign)
                    if partnership_sign
                    else "7th house not available"
                ),
                "relationship_lessons": (
                    get_relationship_lessons(partnership_sign)
                    if partnership_sign
                    else None
                ),
            },
            "compatibility_keys": generate_compatibility_insights(
                venus_info,
                mars_info,
                partnership_sign if partnership_sign is not None else "",
            ),
        }
    ...
    try:
        ...
        career_analysis: Dict[str, Any] = {
            ...
            "mastery_path": {
                ...
                "discipline_area": get_saturn_mastery(
                    saturn_info["sign"], saturn_info["house"]
                ),
            },
            "leadership_style": {
            ...
            "success_formula": synthesize_career_potential(
                mc_sign, saturn_info, sun_info, tenth_house_planets
            ),
        }
    ...
    try:
        ...
        growth_analysis = {
            "saturn_lessons": {
                "saturn_sign": saturn_info["sign"],
                "saturn_house": saturn_info["house"],
                "mastery_challenge": get_saturn_lessons(
                    saturn_info["sign"], saturn_info["house"]
                ),
                "growth_timeline": get_saturn_timeline(),
            },
            "key_challenges": analyze_challenging_aspects(
                challenging_aspects[:5]
            ),
            "integration_work": {
                "primary_tension": identify_primary_tension(
                    challenging_aspects
                ),
                "resolution_path": get_resolution_strategies(
                    challenging_aspects
                ),
            },
            "empowerment_potential": {
                "hidden_strengths": identify_hidden_strengths(chart_data),
                "transformation_gifts": get_transformation_gifts(
                    challenging_aspects
                ),
            },
        }
...
def analyze_spiritual_gifts(chart_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 12th house analysis
        twelfth_house_sign = (
            get_sign_from_position(houses[11]["cusp"])
            if len(houses) >= 12
            else None
        )
        ...
        spiritual_analysis = {
            "psychic_abilities": {
                ...
                "intuitive_gifts": (
                    get_psychic_gifts(twelfth_house_sign)
                    if twelfth_house_sign
                    else None
                ),
            ...
            "mystical_nature": {
                ...
                "spiritual_path": get_neptune_spirituality(
                    neptune_info["sign"], neptune_info["house"]
                ),
            },
            "transformation_power": {
                "pluto_sign": pluto_info["sign"],
                "pluto_house": pluto_info["house"],
                "healing_gifts": get_pluto_healing(
                    pluto_info["sign"], pluto_info["house"]
                ),
            },
            "service_mission": synthesize_spiritual_mission(
                twelfth_house_sign, neptune_info, pluto_info
            ),
        }
    ...
    try:
        ...
        integration_analysis = {
            "elemental_balance": {
                ...
                "dominant_element": (
                    max(element_count.keys(), key=lambda k: element_count[k])
                    if element_count
                    else None
                ),
                "missing_elements": [
                    elem for elem, count in element_count.items() if count == 0
                ],
            ...
            "modal_balance": {
                ...
                "dominant_quality": (
                    max(quality_count.keys(), key=lambda k: quality_count[k])
                    if quality_count
                    else None
                ),
            ...
            "focal_planets": {
                "most_aspected": sorted(
                    planet_aspect_count.items(),
                    key=lambda x: x[1],
                    reverse=True,
                )[:3],
                "integration_priority": get_focal_planet_meaning(
                    planet_aspect_count
                ),
            },
            "overall_theme": synthesize_integration_theme(
                element_count, quality_count, planet_aspect_count
            ),
        }
...
def find_planets_in_house(
    planets: Dict[str, Any], house_number: int
) -> List[str]:
...
def synthesize_life_purpose(
    ...
    return (
        " | ".join(purpose_elements)
        if purpose_elements
        else "Detailed birth data needed for purpose analysis"
    )
...
def get_mc_expression(mc_sign: str) -> str:
    ...
    return (
        f"Publicly expresses {essence} ({archetype})"
        if essence and archetype
        else "MC expression unknown"
    )
...
def get_career_direction(mc_sign: str) -> str:
    ...
    archetype = SIGN_ENERGIES.get(mc_sign, {}).get("archetype", "")
    return (
        f"Career direction: {essence} ({archetype})"
        if essence and archetype
        else "Career direction unknown"
    )
