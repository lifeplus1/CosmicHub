...
ASPECTS: Dict[str, Dict[str, Union[int, float, str]]] = {
    'conjunction': {'angle': 0, 'orb': 8, 'energy': 'intense', 'type': 'major'},
    'opposition': {'angle': 180, 'orb': 8, 'energy': 'challenging', 'type': 'major'},
    'trine': {'angle': 120, 'orb': 6, 'energy': 'harmonious', 'type': 'major'},
    'square': {'angle': 90, 'orb': 6, 'energy': 'challenging', 'type': 'major'},
    'sextile': {'angle': 60, 'orb': 4, 'energy': 'supportive', 'type': 'major'},
    'quincunx': {'angle': 150, 'orb': 3, 'energy': 'adjusting', 'type': 'minor'},
    'semi-sextile': {'angle': 30, 'orb': 2, 'energy': 'subtle', 'type': 'minor'},
    'semi-square': {'angle': 45, 'orb': 2, 'energy': 'tension', 'type': 'minor'},
    'sesquiquadrate': {'angle': 135, 'orb': 2, 'energy': 'friction', 'type': 'minor'},
}
...
LUNAR_PHASES: Dict[str, Dict[str, Union[int, Tuple[int, int], str]]] = {
    'new_moon': {'angle': 0, 'range': (0, 45), 'energy': 'new beginnings', 'description': 'Perfect for setting intentions'},
    'waxing_crescent': {'angle': 45, 'range': (45, 90), 'energy': 'growth', 'description': 'Time for building momentum'},
    'first_quarter': {'angle': 90, 'range': (90, 135), 'energy': 'action', 'description': 'Overcome challenges, take action'},
    'waxing_gibbous': {'angle': 135, 'range': (135, 180), 'energy': 'refinement', 'description': 'Refine and adjust plans'},
    'full_moon': {'angle': 180, 'range': (180, 225), 'energy': 'culmination', 'description': 'Peak energy, completion'},
    'waning_gibbous': {'angle': 225, 'range': (225, 270), 'energy': 'reflection', 'description': 'Release and reflect'},
    'last_quarter': {'angle': 270, 'range': (270, 315), 'energy': 'release', 'description': 'Let go of what no longer serves'},
    'waning_crescent': {'angle': 315, 'range': (315, 360), 'energy': 'rest', 'description': 'Rest and prepare for new cycle'},
}
...
def calculate_planet_position(jd: float, planet_id: int) -> Tuple[float, bool]:
    ...
    if swe:
        try:
            ...
            logger.error(f"SwissEph calculation error for planet {planet_id}: {e}")
            return 0.0, False
    else:
...
def calculate_aspect(pos1: float, pos2: float, orb: float = 2.0) -> Optional[Dict[str, Any]]:
    ...
    for phase_name, phase_data in LUNAR_PHASES.items():
        ...
        if isinstance(range_data, tuple):
            ...
            if range_start <= angle < range_end:
                phase_progress = (angle - range_start) / (range_end - range_start)
                intensity = 100 if phase_name in ['new_moon', 'full_moon'] else 50 + phase_progress * 50
...
def calculate_natal_chart(birth_date: str, birth_time: str, latitude: float, longitude: float) -> Dict[str, float]:
...
def get_cache_key(prefix: str, birth_data: BirthData, date_range: DateRange, **kwargs: Any) -> str:
...
async def calculate_transits(
    ...
    try:
        ...
        if not init_swisseph() and swe_available:
            logger.warning("SwissEph initialization failed, using mock calculations")
        ...
        while current_date <= end_date:
            ...
            for transit_planet, transit_pos in current_positions.items():
                for natal_planet, natal_pos in natal_positions.items():
                    aspect_info = calculate_aspect(transit_pos, natal_pos, request.orb)
                    
                    if aspect_info:
                        # Skip minor aspects if not requested
                        if not request.include_minor_aspects and aspect_info['type'] == 'minor':
                            continue
                        
                        # Calculate duration (simplified)
                        duration_days = 3 if aspect_info['type'] == 'major' else 1
...
async def calculate_lunar_transits(
    ...
    try:
        ...
        if (end_date - start_date).days > 90:
            raise HTTPException(status_code=400, detail="Lunar transit date range cannot exceed 90 days")
        ...
        logger.error(f"Lunar transit calculation error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Lunar transit calculation failed: {str(e)}")
...
async def get_lunar_phase_definitions() -> Dict[str, Any]:
    ...
    return {
        "phases": {name: {k: v for k, v in data.items() if k != 'range'} for name, data in LUNAR_PHASES.items()},
...
async def transit_health_check() -> Dict[str, Any]:
    ...
    return {
        ...
        "swisseph": "available" if swe_status else "unavailable" if swe_available else "not_installed",
