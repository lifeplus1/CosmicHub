    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Overall compatibility: {result['compatibility_analysis']['overall_score']}")
                print(f"Number of interaspects: {len(result['interaspects'])}")
                print(f"Number of house overlays: {len(result['house_overlays'])}")
                print(f"Relationship purpose: {result['composite_chart']['relationship_purpose']}")
                return True
            else:
