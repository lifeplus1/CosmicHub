...
class TransitIntegrationTests:
    ...
    def log_test(self, test_name: str, success: bool, message: str, data: Any = None) -> None:
        """Log test result"""
        result: Dict[str, Any] = {
            "test": test_name,
            "success": success,
    ...
    def test_health_endpoint(self) -> bool:
        ...
        try:
            ...
            message = f"Status: {response.status_code}" if success else f"Failed: {response.status_code}"
    ...
    def test_utility_endpoints(self) -> bool:
        ...
        for endpoint in endpoints:
            try:
                response = requests.get(f"{self.base_url}/api/astro/{endpoint}", timeout=10)
                success = response.status_code == 200
                if success:
                    data = response.json()
                    self.log_test(f"Utility: {endpoint}", True, f"Returned valid data", data)
                else:
                    self.log_test(f"Utility: {endpoint}", False, f"Status: {response.status_code}")
                    all_passed = False
            except Exception as e:
                self.log_test(f"Utility: {endpoint}", False, f"Request failed: {e}")
                all_passed = False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if len(data) > 0:
                        ...
                        required_fields = ["id", "planet", "aspect", "date", "degree", "intensity"]
                        has_required_fields = all(field in sample_transit for field in required_fields)
                        ...
                        if has_required_fields:
                            self.log_test(
                                ...
                                {"count": len(data), "sample": sample_transit, "duration": duration}
                            )
                        else:
                            missing = [f for f in required_fields if f not in sample_transit]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(data) > 0:
                    ...
                    required_fields = ["phase", "date", "energy", "degree", "moon_sign"]
                    has_required_fields = all(field in sample_lunar for field in required_fields)
                    ...
                    if has_required_fields:
                        ...
                        self.log_test(
                            ...
                            {"count": len(data), "phases": unique_phases, "sample": sample_lunar, "duration": duration}
                        )
                        return True
                    else:
                        missing = [f for f in required_fields if f not in sample_lunar]
                        self.log_test("Lunar Transit Calculation", False, f"Missing fields: {missing}")
                        return False
                else:
                    self.log_test("Lunar Transit Calculation", False, "No lunar transit results returned")
                    return False
            else:
                error_data = response.json() if response.headers.get('content-type', '').startswith('application/json') else response.text
                self.log_test("Lunar Transit Calculation", False, f"Status: {response.status_code}", error_data)
                return False
                
        except Exception as e:
            self.log_test("Lunar Transit Calculation", False, f"Request failed: {e}")
            return False
    ...
    def test_performance_and_caching(self) -> bool:
        ...
        try:
            ...
            if response1.status_code != 200:
                self.log_test("Performance Test", False, f"First request failed: {response1.status_code}")
                return False
            ...
            if response2.status_code != 200:
                self.log_test("Performance Test", False, f"Second request failed: {response2.status_code}")
                return False
            ...
            performance_improved = duration2 <= duration1 * 1.2  # Allow some variance
            reasonable_performance = duration1 < 10.0  # Should complete within 10 seconds
            ...
            self.log_test("Performance & Caching", False, f"Performance test failed: {e}")
            return False
...
if __name__ == "__main__":
    ...
    with open("/Users/Chris/Projects/CosmicHub/backend/integration_test_results.json", "w") as f:
