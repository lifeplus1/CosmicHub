...
class InputValidator:
    ...
    def validate_frequency_settings(settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'duration' in settings:
            ...
            if not isinstance(duration, (int, float)) or not (1 <= duration <= 120):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
...
general_limiter = RateLimiter(max_requests=100, window_seconds=3600)  # 100 requests per hour
auth_limiter = RateLimiter(max_requests=10, window_seconds=900)       # 10 auth attempts per 15 minutes
heavy_limiter = RateLimiter(max_requests=20, window_seconds=3600)     # 20 heavy operations per hour
