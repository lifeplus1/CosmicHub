...
from utils.vectorized_memory_optimization import (
    MemoryOptimizedVectorizedCalculator, memory_optimized_processing, MemoryMonitor
)
from utils.vectorized_caching import (
    TieredCacheManager, cached_calculation, ChartDataHasher, get_global_cache_manager
)
...
class OptimizedVectorizedAspectCalculator:
    ...
    def __init__(self,
        ...
        self.performance_monitor = VectorizedPerformanceMonitor() if enable_monitoring else None
        
        self.cache_manager = cache_manager or (get_global_cache_manager() if enable_caching else None)
        ...
        logger.info(f"Initialized OptimizedVectorizedAspectCalculator with "
                   f"chunk_size={chunk_size}, caching={enable_caching}, "
                   f"monitoring={enable_monitoring}")
    
    def _get_cache_key_data(self, chart1: Dict[str, float], chart2: Dict[str, float]) -> Tuple[str, str]:
    ...
    def calculate_synastry_aspects(self, 
                                 chart1: Dict[str, float], 
                                 chart2: Dict[str, float],
                                 orb: float = 8.0,
                                 aspects: Optional[List[str]] = None) -> np.ndarray:
        """
        ...
        if self.enable_monitoring and self.performance_monitor:
            with self.performance_monitor.time_operation(operation_name):
                result = self._compute_synastry_aspects(chart1, chart2, orb, aspects)
    ...
    def _compute_synastry_aspects(self, chart1: Dict[str, float], chart2: Dict[str, float], 
                                orb: float, aspects: List[str]) -> np.ndarray:
        ...
        try:
            ...
            logger.warning("VectorizedAspectCalculator not available, using fallback")
            return self._fallback_aspect_calculation(chart1, chart2, orb)
    
    def _fallback_aspect_calculation(self, chart1: Dict[str, float], chart2: Dict[str, float], 
                                   orb: float) -> np.ndarray:
    ...
    def calculate_large_batch_synastry(self, 
        ...
        logger.info(f"Starting batch synastry calculation for {total_pairs} chart pairs")
        ...
        with memory_optimized_processing(
            ...
            current_mb = memory_stats.get('current_memory_mb', memory_stats.get('memory_increase_mb', 0.0))
            peak_mb = memory_stats.get('peak_memory_mb', memory_stats.get('peak_increase_mb', current_mb))
            logger.info(f"Batch processing completed. Memory used: {current_mb:.2f}MB, "
                       f"Peak: {peak_mb:.2f}MB")
            
            return results
    
    def _process_batch_with_caching(self, 
                                  chart_pairs: List[Tuple[Dict[str, float], Dict[str, float]]],
                                  orb: float,
                                  progress_callback: Optional[Callable],
                                  memory_calculator) -> List[np.ndarray]:
        ...
        cache_hit_rate = (cache_hits / len(chart_pairs)) * 100 if chart_pairs else 0
        logger.info(f"Batch processing: {cache_hits}/{len(chart_pairs)} cache hits ({cache_hit_rate:.1f}%)")
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self.enable_monitoring and self.performance_monitor:
            ...
            metrics['performance'] = {
                'operations_monitored': len(self.performance_monitor.metrics_history) if hasattr(self.performance_monitor, 'metrics_history') else 0
            }
    ...
    def optimize_settings_for_dataset(self, dataset_size: int, available_memory_mb: float) -> Dict[str, Any]:
        """
        ...
        estimates = self.memory_calculator.estimate_memory_usage(
            num_charts1=dataset_size,
            num_charts2=dataset_size
        )
        ...
        recommendations = {
            ...
            'enable_caching': dataset_size > 100,  # Cache beneficial for larger datasets
            'enable_memory_pooling': dataset_size > 50,  # Pooling beneficial for medium+ datasets
            'expected_memory_usage': estimates,
            'cache_hit_rate_estimate': min(80.0, dataset_size / 10)  # Rough estimate
        }
...
def optimized_synastry_calculation(chart1: Dict[str, float], 
                                 chart2: Dict[str, float],
                                 orb: float = 8.0,
    ...
    return {
        ...
        'metadata': {
            'orb': orb,
            'cached': 'cache' in metrics and metrics['cache']['memory_cache']['hits'] > 0,
...
@contextmanager
def optimized_calculation_session(chunk_size: int = 100, 
                                enable_caching: bool = True,
                                enable_monitoring: bool = True):
    """
    ...
    try:
        ...
        if enable_monitoring:
            ...
            if 'cache' in metrics:
                ...
                logger.info(f"Session complete: {cache_stats['hits']} cache hits, "
                           f"{cache_stats['hit_rate']:.1f}% hit rate")

# Utility functions for migration from traditional calculations

def migrate_traditional_to_optimized(traditional_function: Callable,
    ...
    def wrapper(*args, **kwargs):
        ...
        if enable_performance_comparison:
            ...
            if len(args) >= 2:
                optimized_result = calculator.calculate_synastry_aspects(args[0], args[1])
            else:
            ...
            logger.info(f"Performance comparison - Traditional: {traditional_time:.3f}s, "
