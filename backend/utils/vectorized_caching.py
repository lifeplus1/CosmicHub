...
class CacheKey:
    ...
    def __str__(self) -> str:
        return f"{self.calculation_type}:{self.chart_hash}:{self.parameters_hash}"
    ...
    def _evict_lru(self):
        ...
        with self.lock:
            while (len(self.cache) > self.max_size or 
                   self.stats.size_mb > self.max_memory_mb):
                if not self.cache:
                    break
...
class TieredCacheManager:
    ...
    def __init__(self, 
        ...
        self.persistent_cache = PersistentCache(persistent_cache_dir, persistent_max_age_hours) if enable_persistent else None
        self.hasher = ChartDataHasher()
        self.lock = threading.Lock()
    
    def _create_cache_key(self, calculation_type: str, 
    ...
    def get(self, calculation_type: str, 
           chart_data: Optional[Dict[str, float]] = None,
           chart_data_hash: Optional[str] = None,
           **kwargs) -> Optional[Any]:
        """Get cached value, checking memory first, then persistent."""
        with self.lock:
            key = self._create_cache_key(calculation_type, chart_data, chart_data_hash, **kwargs)
    ...
    def put(self, value: Any, calculation_type: str,
           chart_data: Optional[Dict[str, float]] = None,
           chart_data_hash: Optional[str] = None,
           **kwargs):
...
def cached_calculation(cache_manager: TieredCacheManager, 
                      calculation_type: str,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if cache_chart_data:
                ...
                for param_name in ['chart1', 'chart2', 'chart_data', 'planets']:
...
@contextmanager
def cached_computation(calculation_type: str, 
                      chart_data: Optional[Dict[str, float]] = None,
                      **kwargs):
    ...
    # Check cache first
    cached_result = cache_manager.get(calculation_type, chart_data=chart_data, **kwargs)
...
if __name__ == "__main__":
    ...
    # Cache some data
    cache_manager.put("test_data", "aspect_calculation", chart_data=test_chart, orb=8.0)
    
    # Retrieve cached data
    result = cache_manager.get("aspect_calculation", chart_data=test_chart, orb=8.0)
