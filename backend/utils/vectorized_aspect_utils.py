...
from .aspect_utils import PLANETS, ASPECT_DEGREES, ORBS, AspectData
...
class VectorizedAspectCalculator:
    ...
    def find_aspects_vectorized(self, separations: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        best_aspect_indices = np.argmin(
            np.where(valid_aspects, orbs_matrix, np.inf), 
            axis=2
        )
    ...
    def build_aspect_matrix_vectorized(
        ...
        aspect_indices, orbs, has_aspect = self.find_aspects_vectorized(separations)
    ...
    def batch_compatibility_scores(
        ...
        for separations in all_separations:
            aspect_indices, orbs, has_aspect = self.find_aspects_vectorized(separations)
            score = self._calculate_score_vectorized(aspect_indices, orbs, has_aspect)
            scores.append(score)
    ...
    def _calculate_score_vectorized(
        ...
        aspect_scores = np.array([4, 3, 2, 1, -2, -3, -1])  # matches ASPECT_SCORES order
        planet_weights = np.array([3, 3, 2, 3, 3, 2, 1.5, 1, 1, 1])  # PLANET_WEIGHTS order
        
        # Calculate weights matrix
        weight_matrix = (planet_weights[:, np.newaxis] + planet_weights[np.newaxis, :]) / 2
...
def batch_synastry_analysis(
    ...
    for i, (long1, long2) in enumerate(chart_pairs):
        matrix = vectorized_calculator.build_aspect_matrix_vectorized(long1, long2)
        results.append({
            'compatibility_score': scores[i],
