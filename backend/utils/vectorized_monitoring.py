...
class VectorizedPerformanceMonitor:
    ...
    def __init__(self, enable_detailed_logging: bool = True, metrics_retention_hours: float = 24):
    ...
    def time_operation(self, 
                      ...
                      array_sizes: Optional[List[int]] = None) -> Generator['OperationContext', None, None]:
        """
        ...
        """
        context = OperationContext(operation_name, num_calculations, array_sizes or [])
    ...
    def detect_performance_regression(self, 
                                    operation_name: str, 
                                    threshold_factor: float = 1.5) -> Optional[Dict[str, Any]]:
        """
        ...
        recent_metrics = [
            m for m in self.metrics_history[-10:]  # Last 10 operations
        ...
        recent_avg_memory = sum(abs(m.memory_delta_mb) for m in recent_metrics) / len(recent_metrics)
        
        duration_regression = recent_avg_duration > (baseline['avg_duration_ms'] * threshold_factor)
        memory_regression = recent_avg_memory > (baseline['avg_memory_delta_mb'] * threshold_factor)
    ...
    def generate_performance_report(self, operation_filter: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        if operation_filter:
            relevant_metrics = [m for m in self.metrics_history if m.operation_name == operation_filter]
        else:
        ...
        if successful_metrics:
            ...
            memory_deltas = [abs(m.memory_delta_mb) for m in successful_metrics]
            calculations_per_sec = [m.calculations_per_second for m in successful_metrics]
            ...
            throughput_stats = {
                ...
                'avg_calc_per_sec': sum(calculations_per_sec) / len(calculations_per_sec)
            }
        else:
            duration_stats = memory_stats = throughput_stats = {}
        
        # Recent performance trend
        recent_metrics = successful_metrics[-20:] if len(successful_metrics) >= 20 else successful_metrics
    ...
    def export_metrics(self, filepath: Union[str, Path], format_type: str = 'json'):
        """
        ...
        if format_type == 'json':
            ...
            with open(filepath, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    'timestamp', 'operation_name', 'duration_ms', 'memory_delta_mb',
                    'num_calculations', 'calculations_per_second', 'success',
                    'error_message', 'array_sizes_json'
                ])
...
class OperationContext:
    ...
    def __init__(self, operation_name: str, num_calculations: int, array_sizes: List[int]):
