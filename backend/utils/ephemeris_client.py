...
class CalculationResponse(BaseModel):
    ...
    calculation_time: datetime = Field(..., description="UTC time of calculation")

class BatchCalculationRequest(BaseModel):
    """Request model for batch planetary position calculations."""
    calculations: List[CalculationRequest] = Field(..., description="List of calculations")
...
class BatchCalculationResponse(BaseModel):
    ...
    calculation_time: datetime = Field(..., description="UTC time of batch calculation")
...
class EphemerisClient:
    ...
    def __init__(
        ...
        """
        self.server_url = server_url or os.getenv('EPHEMERIS_SERVER_URL', 'http://localhost:8001')
        self.api_key = api_key or os.getenv('API_KEY')
        self.timeout = timeout
        self.cache_ttl = cache_ttl
        
        if not self.api_key:
            raise EphemerisClientError("API key is required for ephemeris server authentication")
    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            raise EphemerisClientError(f"HTTP {e.response.status_code}: {e.response.text}")
    ...
    async def calculate_position(self, julian_day: float, planet: str) -> CalculationResponse:
        """
        ...
        """
        # Check cache first
        cache_key = self._get_cache_key('calculate', f"{julian_day}:{planet.lower()}")
        ...
        logger.debug(f"Calculated {planet} position: {result.position.position:.6f}°")
        return result
    ...
    async def calculate_batch_positions(
        ...
        for calc_result in result.results:
            cache_key = self._get_cache_key(
                'calculate', 
                f"{calc_result.julian_day}:{calc_result.planet}"
            )
    ...
    async def fetch_ephemeris_data(self, filename: str) -> bytes:
        ...
        try:
            ...
            raise EphemerisClientError(f"Failed to fetch {filename}: HTTP {e.response.status_code}")
        except Exception as e:
            logger.error(f"Error fetching {filename}: {e}")
            raise EphemerisClientError(f"Failed to fetch {filename}: {e}")

# Convenience functions for backward compatibility
async def get_planetary_positions(julian_day: float) -> Dict[str, PlanetPosition]:
    """
...
async def calculate_single_position(julian_day: float, planet: str) -> Optional[PlanetPosition]:
    """
