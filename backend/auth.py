    ...
    try:
        ...
        try:
            ...
            if allow_mock in ("1", "true", "yes") and env != "production":
                firebase_available = False
                logger.warning(
                    "Firebase Admin credentials not found. Running with mock auth "
...
async def get_current_user(
    ...
    try:
        ...
        uid: Any = decoded_token.get("uid")  # type: ignore[misc]
        if not uid or not isinstance(uid, str):
            logger.error("No valid UID found in decoded token")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
