    ...
    async tryFirebasePerformanceImport(name, duration, metadata) {
        try {
            ...
            if (import.meta.env.DEV) {
                logger.warn('Firebase Performance Monitoring not available, skipping trace', { operation: name });
            }
        }
    }
}
...
export const initServiceWorkerPerformanceCache = () => {
    if ('serviceWorker' in navigator && process.env.NODE_ENV === 'production') {
        navigator.serviceWorker.register('/performance-sw.js')
            .then(registration => {
            logger.info('Performance service worker registered', { scope: registration.scope });
        })
            .catch(error => {
