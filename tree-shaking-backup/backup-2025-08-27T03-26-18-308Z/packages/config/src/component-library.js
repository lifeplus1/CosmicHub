...
export function createCompoundComponent(Base, name) {
  const Header = ({ children, className = '', ...rest }) =>
    _jsx('div', {
      className: `compound-header ${className}`,
      ...rest,
      children: children,
    });
  const Body = ({ children, className = '', ...rest }) =>
    _jsx('div', {
      className: `compound-body ${className}`,
      ...rest,
      children: children,
    });
  const Footer = ({ children, className = '', ...rest }) =>
    _jsx('div', {
      className: `compound-footer ${className}`,
      ...rest,
      children: children,
    });
  const Actions = ({ children, className = '', ...rest }) =>
    _jsx('div', {
      className: `compound-actions ${className}`,
      ...rest,
      children: children,
    });
...
export function useComponentAnalysis(name) {
  ...
  const recordMetric = useCallback(
    (metric, value) => analyzer.recordComponentMetric(name, metric, value),
    [analyzer, name]
  );
  return {
    recordMetric,
  ...
  const Composed = props => {
    if (strategy === 'sequential')
      return _jsx(_Fragment, {
        children: components.map((C, i) => _jsx(C, { ...props }, i)),
      });
    if (strategy === 'parallel')
      return _jsx('div', {
        className: 'composition-parallel',
        children: components.map((C, i) =>
          _jsx('div', { children: _jsx(C, { ...props }) }, i)
        ),
      });
...
export function withMemoization(Component, strategy = { shallow: true }) {
  if (strategy.custom) return memo(Component, strategy.custom);
