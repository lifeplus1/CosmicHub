...
export const DefaultLoadingSpinner = () => (_jsx("div", { className: "flex items-center justify-center p-8", children: _jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-cosmic-purple" }) }));
export const PageLoadingSpinner = () => (_jsx("div", { className: "min-h-screen flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-b-2 border-cosmic-purple mx-auto mb-4" }), _jsx("p", { className: "text-gray-600", children: "Loading..." })] }) }));
// Enhanced lazy loading with performance tracking
export function createLazyComponent(importFn, componentName, options = {}) {
    const { loadingComponent: LoadingComponent = DefaultLoadingSpinner, preload = false, timeout = 10000 } = options;
    ...
    const WrappedComponent = React.forwardRef((props, ref) => (_jsx(Suspense, { fallback: _jsx(LoadingComponent, {}), children: _jsx(LazyComponent, { ...props, ref: ref }) })));
    WrappedComponent.displayName = `Lazy(${componentName})`;
    return WrappedComponent;
}
// Lazy loading utilities for common patterns
export const lazyLoadRoute = (importFn, routeName) => (createLazyComponent(importFn, `Route_${routeName}`, {
    loadingComponent: PageLoadingSpinner,
    preload: false,
    timeout: 15000
}));
export const lazyLoadModal = (importFn, modalName) => (createLazyComponent(importFn, `Modal_${modalName}`, {
    loadingComponent: DefaultLoadingSpinner,
    preload: true,
    timeout: 5000
}));
export const lazyLoadChart = (importFn, chartName) => (createLazyComponent(importFn, `Chart_${chartName}`, {
    loadingComponent: DefaultLoadingSpinner,
    preload: false,
    timeout: 8000
}));
...
const DefaultErrorFallback = ({ error, resetError }) => (_jsxs("div", { className: "p-4 border border-red-200 rounded-lg bg-red-50", children: [_jsx("h3", { className: "text-red-800 font-semibold mb-2", children: "Component Loading Error" }), _jsx("p", { className: "text-red-600 text-sm mb-3", children: error.message }), _jsx("button", { onClick: resetError, className: "px-3 py-1 bg-red-600 text-white rounded text-sm hover:bg-red-700", children: "Retry" })] }));
...
export function useLazyLoading() {
    ...
    const isLoading = React.useCallback((componentName) => {
