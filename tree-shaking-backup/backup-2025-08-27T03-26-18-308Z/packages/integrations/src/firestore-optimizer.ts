 ...
 */
export function withFirestoreTracking<TArgs extends unknown[], TResult>(
  querySignature: string,
  fn: (...args: TArgs) => Promise<TResult>,
  readCountExtractor?: (result: TResult) => number
...
export class OptimizedFirestoreQuery {
  ...
  private estimatedReads = 1;

  constructor(collection: string) {
    this.querySignature = `collection_${collection}`;
  }

  where(field: string, operator: string, _value: unknown): this {
    this.querySignature += `_where_${field}_${operator}`;
    return this;
  }

  orderBy(field: string, direction: 'asc' | 'desc' = 'asc'): this {
    this.querySignature += `_order_by_${field}_${direction}`;
    return this;
  }

  limit(count: number): this {
    this.querySignature += `_limit_${count}`;
    this.estimatedReads = Math.min(this.estimatedReads, count);
    return this;
  }

  async execute<T>(executor: () => Promise<T>): Promise<T> {
    const wrappedExecutor = withFirestoreTracking(
      this.querySignature,
      executor,
      () => this.estimatedReads
    );
