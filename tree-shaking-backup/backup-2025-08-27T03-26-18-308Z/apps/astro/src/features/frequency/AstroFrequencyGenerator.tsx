 ...
 */
const AstroFrequencyGenerator: React.FC<AstroFrequencyGeneratorProps> =
  React.memo(({ chartData }) => {
    ...
    const getPersonalizedFrequency = useCallback(
      (basePreset: AstroFrequencyPreset): AstroFrequencyPreset => {
        ...
        const dominantElement =
          typeof dominantRaw === 'string' && dominantRaw.length > 0
            ? dominantRaw
            : 'earth';
    ...
    return (
      <div
        className='astro-frequency-generator'
        role='region'
        aria-label='Astro Frequency Generator'
      >
        <h2 className='mb-6 text-2xl font-bold'>
          🌟 Astrology-Enhanced Frequency Therapy
        </h2>
        ...
        {chartData !== null && chartData !== undefined && (
          <div className='p-4 mb-6 border border-purple-200 rounded-lg bg-purple-50'>
            ...
            <p className='mt-1 text-sm text-purple-700'>
              Frequencies are automatically adjusted based on your dominant
              element and current transits
            </p>
          </div>
        )}
              ...
              {astroEnhancedPresets.map(preset => {
                ...
                return (
                  <Tooltip.Root key={preset.id}>
                    ...
                    <Tooltip.Portal>
                      <Tooltip.Content className='p-2 bg-white border rounded shadow'>
                        Premium: Integrate with HealWave for more
                      </Tooltip.Content>
                    </Tooltip.Portal>
                  </Tooltip.Root>
                );
              ...
              <div>
                ...
                <Slider.Root
                  ...
                  onValueChange={([value]) =>
                    updateSettings(
                      'volume',
                      typeof value === 'number' ? value : settings.volume
                    )
                  }
              ...
              <div>
                ...
                <Slider.Root
                  ...
                  onValueChange={([value]) =>
                    updateSettings(
                      'duration',
                      typeof value === 'number' ? value : settings.duration
                    )
                  }
                ...
                {selectedPreset.astrologyData !== null &&
                  selectedPreset.astrologyData !== undefined && (
                    <div className='mt-1 text-xs text-purple-600'>
                      Recommended:{' '}
                      {selectedPreset.astrologyData.recommendedDuration} min
                    </div>
                  )}
              </div>
            </div>
        ...
        <Suspense fallback={<div>Loading...</div>}>
          {selectedPreset !== null && selectedPreset !== undefined && (
            <LazyAstroInfo preset={selectedPreset} />
          )}
        </Suspense>
      </div>
    );
