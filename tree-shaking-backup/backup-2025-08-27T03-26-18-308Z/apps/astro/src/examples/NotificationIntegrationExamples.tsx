    ...
    try {
      ...
      const maybeSync = (
        notificationManager as unknown as {
          syncChartCalculation?: (data: {
            userId: string;
            birthData: BirthData;
            timestamp: number;
          }) => Promise<void>;
        }
      ).syncChartCalculation;
...
export const UserSettingsWithNotifications: React.FC = () => {
  ...
  useEffect((): (() => void) => {
    ...
    const loadStatus = (): void => {
      const raw = notificationManager.status() as unknown as {
      ...
      const permissionStatus =
        typeof push['permissionStatus'] === 'string'
      ...
      const pushNotifications =
        permissionStatus !== undefined && permissionStatus !== ''
          ...
          : undefined;
      const isOnline =
        typeof background['isOnline'] === 'boolean'
          ? background['isOnline']
          : undefined;
      const backgroundSync =
        isOnline !== undefined
          ? {
              isOnline,
              queuedItems:
                typeof background['queuedItems'] === 'number'
                  ? background['queuedItems']
                  : undefined,
            }
          : undefined;
  ...
  return (
    <div className='user-settings'>
      ...
      <div className='notification-actions'>
        ...
        <button
          onClick={(): void => {
            void notificationManager.sendTest();
          }}
  ...
  useEffect((): (() => void) => {
    ...
    const handleStorageChange = (event: StorageEvent): void => {
      if (
        ...
        event.key.startsWith('cosmichub-sync-') &&
        ...
        try {
          ...
          if (
            maybe.type === 'cosmichub-sync-chart_synced' ||
            maybe.type === 'cosmichub-sync-user_data_synced'
          ) {
            if (typeof maybe.timestamp !== 'number') return;
            const msg: StorageMessage = {
              type: maybe.type,
            ...
            setNotifications((prev: NotificationItem[]): NotificationItem[] => [
              {
                id: Date.now(),
                type:
                  msg.type === 'cosmichub-sync-chart_synced'
                    ? 'chart_synced'
                    : 'user_data_synced',
                data:
                  msg.data !== undefined &&
    ...
    <div className='dashboard'>
      <div className='notification-feed mb-6'>
        ...
        <div className='space-y-2'>
          {notifications.map(
            (notification: NotificationItem): ReactElement => (
              <div
                key={notification.id}
                ...
                <div className='flex justify-between items-start'>
                  <div>
                    <div className='font-medium'>
                      {notification.type === 'chart_synced' &&
                        '📊 Chart Synchronized'}
                      {notification.type === 'user_data_synced' &&
                        '👤 Profile Updated'}
                    </div>
                  ...
                  <div className='text-xs text-blue-600'>✅ Synced</div>
                </div>
              </div>
            )
          )}
        ...
        <div className='space-x-4'>
          ...
          <button
            onClick={(): void => {
              ...
              devConsole.debug?.(
                'Retrograde alert demo: Mercury entering retrograde'
              );
            }}
            className='px-4 py-2 bg-purple-600 text-white rounded'
          >
...
export const useNotifications = (): UseNotificationsReturn => {
  ...
  useEffect((): (() => void) => {
    const updateStatus = (): void => {
      const raw = manager.status() as unknown as {
        push?: Record<string, unknown>;
        background?: Record<string, unknown>;
      };
      const push = raw.push ?? {};
  ...
  return {
    ...
    subscribe: (
      userId: string,
      preferences?: UserPreferences
    ): Promise<boolean> => manager.subscribe(userId, preferences),
