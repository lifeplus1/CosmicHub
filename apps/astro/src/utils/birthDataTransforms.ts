...
export function toCanonicalBirthData(b: ExtendedBirthData | ChartBirthData): ChartBirthData {
...
export function hasCompleteCanonicalBirthData(b: Partial<ChartBirthData>): b is ChartBirthData {
  return typeof b.birth_date === 'string' && typeof b.birth_time === 'string' && typeof b.latitude === 'number' && typeof b.longitude === 'number';
}
...
export interface RawNumericBirthData {
  year: number; month: number; day: number; hour: number; minute: number; city: string;
  lat?: number; lon?: number; timezone?: string;
}

export function isRawNumericBirthData(data: unknown): data is RawNumericBirthData {
  if (data === null || typeof data !== 'object') return false;
  const o = data as Record<string, unknown>;
  return [o.year,o.month,o.day,o.hour,o.minute].every(v=>Number.isInteger(v)) && typeof o.city === 'string';
}

export function parseBirthParams(sp: URLSearchParams): RawNumericBirthData | null {
  const required = ['year','month','day','hour','minute','city'] as const;
  for (const key of required) { const v = sp.get(key); if (!v) return null; }
  ...
  const candidate: RawNumericBirthData = { year, month, day, hour, minute, city, lat, lon, timezone };
