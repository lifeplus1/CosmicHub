...
import {
  toCanonicalBirthData,
  parseBirthParams,
  hasCompleteCanonicalBirthData,
} from '../birthDataTransforms';
...
describe('birthDataTransforms', () => {
  ...
  it('parseBirthParams returns null when required missing', () => {
    const params = new URLSearchParams(
      'year=2000&month=1&day=2&hour=3&minute=4'
    );
  ...
  it('parseBirthParams parses full set including optional lat/lon/timezone', () => {
    const params = new URLSearchParams(
      'year=2000&month=1&day=2&hour=3&minute=4&city=Paris&lat=48.8566&lon=2.3522&timezone=Europe/Paris'
    );
