...
const BlogSubscription: React.FC<BlogSubscriptionProps> = ({ 
  ...
  if (!email?.includes('@')) {
    ...
    try {
      ...
      setStatus('success');
      setMessage('Welcome to the cosmic community! Check your email for confirmation.');
      setEmail('');
  ...
  if (variant === 'sidebar') {
    return (
      <div className={`${baseClasses} bg-gradient-to-br from-cosmic-purple/10 to-cosmic-blue/10 rounded-xl p-6 border border-cosmic-silver/10 sticky top-8`}>
      ...
      <div className={`${baseClasses} max-w-md mx-auto`}>
        <div className="text-center mb-6">
          ...
          <p className="text-cosmic-silver/80">
            Join thousands of cosmic explorers receiving weekly wisdom, exclusive content, and early access to new features.
          </p>
        </div>
  ...
  return (
    <div className={`${baseClasses} bg-gradient-to-r from-cosmic-purple/20 to-cosmic-blue/20 rounded-2xl p-8 border border-cosmic-silver/10`}>
      <div className="max-w-2xl mx-auto text-center">
        ...
        <p className="text-cosmic-silver mb-6">
          Get the latest insights on astrology, numerology, human design, and consciousness delivered weekly. 
          Join our community of cosmic explorers and never miss a revelation.
        </p>
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={(e) => { void handleSubmit(e); }} className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
