  ...
  ({ user, userTier, handleSignOut }) => {
    ...
    const getTierBadge = (tier: string) => {
      switch (tier) {
        case 'elite':
          return (
            <span className='px-2 py-1 text-xs font-bold text-gold-900 bg-gradient-to-r from-gold-400 to-gold-600 rounded-full'>
              ELITE
            </span>
          );
        case 'premium':
          return (
            <span className='px-2 py-1 text-xs font-bold text-purple-900 bg-gradient-to-r from-purple-400 to-purple-600 rounded-full'>
              PRO
            </span>
          );
        default:
        ...
        <DropdownMenu.Trigger asChild>
          ...
          >
            ...
            <div className='flex flex-col items-start'>
              <span className='text-sm font-medium text-cosmic-silver truncate max-w-32'>
                {user.email}
              </span>
              {getTierBadge(userTier)}
            </div>
            <svg
              className='w-4 h-4 text-cosmic-silver'
              fill='none'
              stroke='currentColor'
              viewBox='0 0 24 24'
            >
