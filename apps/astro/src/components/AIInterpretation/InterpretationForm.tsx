...
const InterpretationForm: React.FC<InterpretationFormProps> = ({
  ...
  const showToast = useCallback((opts: { title: string; description: string; status: 'success' | 'error'; duration?: number }) => {
    toast({ ...opts, duration: opts.duration ?? (opts.status === 'success' ? 3000 : 5000), isClosable: true });
  }, [toast]);
  ...
  const handleFocusToggle = (focusLabel: string): void => {
    ...
    setChartForm(prev => {
      ...
      return { ...prev, focus: exists ? prev.focus.filter(f => f !== canonical) : [...prev.focus, canonical] };
    });
  };
  ...
  const handleChartGenerate = async (): Promise<void> => {
    ...
    if (mode === 'chart' && (chartId === undefined || chartId === null || chartId === '')) {
      showToast({
    ...
    if (isSynastry) {
      const synastryMissing = !partnerBirthDate || !partnerBirthTime || !partnerBirthLocation;
      if (synastryMissing) {
        showToast({
  ...
  const handleGenerate = (): Promise<void> => (mode === 'chart' ? handleChartGenerate() : handleDirectGenerate());
  ...
  {mode === 'direct' && (
          <>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              <div>
                ...
                <input
                  ...
                  onChange={(e): void => setDirectForm(prev => ({ ...prev, birthDate: e.target.value }))}
                ...
                <p id='birth-date-format' className='sr-only'>Format: YYYY-MM-DD</p>
                {directForm.birthDate !== '' && !isValidDate(directForm.birthDate) && (
                  <p id='birth-date-error' className='mt-1 text-xs text-red-400'>Invalid date format. Use YYYY-MM-DD.</p>
                )}
              </div>
              <div>
                <label
                ...
                <input
                  ...
                  onChange={(e): void => setDirectForm(prev => ({ ...prev, birthTime: e.target.value }))}
                ...
                <p id='birth-time-format' className='sr-only'>Format: 24-hour HH:MM</p>
                {directForm.birthTime !== '' && !isValidTime(directForm.birthTime) && (
                  <p id='birth-time-error' className='mt-1 text-xs text-red-400'>Invalid time format. Use 24h HH:MM.</p>
                )}
              </div>
              <div>
                <label
                ...
                <input
                  ...
                  onChange={(e): void => setDirectForm(prev => ({ ...prev, birthLocation: e.target.value }))}
            ...
            <div>
              ...
              <fieldset className='grid grid-cols-1 md:grid-cols-2 gap-3' aria-labelledby='direct-interpretation-type-legend'>
                <legend id='direct-interpretation-type-legend' className='sr-only'>Select AI interpretation focus</legend>
                {aiInterpretationTypes.map(type => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e): void => setDirectForm(prev => ({ ...prev, interpretationType: e.target.value as DirectFormState['interpretationType'] }))}
                      className='sr-only'
  ...
  {mode === 'chart' && (
          <>
            ...
            <div>
              ...
              <fieldset className='grid grid-cols-1 md:grid-cols-2 gap-3' aria-labelledby='chart-interpretation-type-legend'>
                <legend id='chart-interpretation-type-legend' className='sr-only'>Options for selecting chart mode</legend>
                {interpretationTypes.map(type => (
                  ...
                  >
                    <input
                      ...
                      onChange={e => setChartForm(prev => ({ ...prev, type: e.target.value as InterpretationType }))}
                      className='sr-only'
      ...
      <fieldset className='flex flex-wrap gap-2' aria-labelledby='focus-area-legend'>
        <legend id='focus-area-legend' className='sr-only'>Focus area toggle buttons</legend>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={e => setChartForm(prev => ({ ...prev, question: e.target.value }))}
            ...
            {isSynastry && (
              <div className='space-y-4'>
                <div className='text-cosmic-gold font-medium'>Partner Birth Data (Synastry)</div>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  <div>
                    <label htmlFor='partner-birth-date' className='block text-cosmic-gold text-sm mb-1'>Partner Birth Date</label>
                    <input
                  ...
                  <div>
                    <label htmlFor='partner-birth-time' className='block text-cosmic-gold text-sm mb-1'>Partner Birth Time</label>
                    <input
                  ...
                  <div>
                    <label htmlFor='partner-birth-location' className='block text-cosmic-gold text-sm mb-1'>Partner Birth Location</label>
                    <input
        ...
        {interpretation && (
          <div className='p-4 bg-cosmic-purple/10 border border-cosmic-purple/30 rounded-lg' aria-live='polite'>
