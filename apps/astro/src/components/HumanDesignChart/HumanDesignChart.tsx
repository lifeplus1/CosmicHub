...
const HumanDesignChart: React.FC<HumanDesignChartProps> = ({
  birthData,
  onCalculate,
  onHumanDesignCalculated,
}) => {
  const [humanDesignData, setHumanDesignData] =
    useState<HumanDesignData | null>(null);
  ...
  const handleCalculate = useCallback(async (): Promise<void> => {
    ...
    try {
      const result: ApiResult<{ human_design: HumanDesignData }> =
        await calculateHumanDesign(birthData);
      if (result.success) {
        ...
        toast({
          ...
          description:
            'Your Human Design chart has been generated successfully!',
          status: 'success',
          duration: 3000,
        });
      } else {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to calculate Human Design';
    ...
    return (
      <div className='p-8 cosmic-card'>
        <div className='space-y-4 text-center'>
          <div className='mx-auto text-4xl text-purple-500 animate-spin'>
            ⭐
          </div>
          <p className='text-lg font-medium text-cosmic-silver'>
            Calculating your Human Design chart...
          </p>
        </div>
      </div>
    );
  ...
  const typeValue: string =
    typeof humanDesignData.type === 'string' &&
    humanDesignData.type.trim().length > 0
  ...
  return (
    <div className='max-w-6xl mx-auto space-y-6'>
        ...
        <div className='p-6'>
          <div className='flex items-center justify-between mb-6'>
            ...
            <div className='flex flex-col items-end space-y-2'>
              ...
              <p className='text-lg font-bold text-cosmic-gold'>
                {humanDesignData.strategy}
              </p>
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 gap-6 md:grid-cols-3'>
            <div className='text-center md:text-left'>
              ...
              <p className='text-lg font-bold text-cosmic-silver'>
      ...
      {/* Tabbed Content */}
      <Tabs.Root
        className='border rounded-md border-cosmic-silver/30'
