...
const HumanDesignModal: React.FC<HumanDesignModalProps> = ({
  isOpen,
  onClose,
  data,
}) => {
      ...
      <div className='space-y-4'>
        ...
        <div className='mt-2'>
          ...
          <p className='text-cosmic-text text-sm'>
            This line filters the gate&apos;s archetype through a specific role
            expression. Combine this keynote with your authority &amp; strategy
        ...
        <div className='mt-6 p-4 bg-cosmic-purple/10 border border-cosmic-purple/30 rounded-lg'>
          ...
          <p className='text-cosmic-text text-sm'>
            This gate carries the archetypal energy of{' '}
            {gateData.name.toLowerCase()}, activated through your{' '}
            {gateData.planet} in{' '}
            {gateData.gateType === 'personality' ? 'conscious' : 'unconscious'}{' '}
            expression.
          </p>
        </div>
      </div>
    ...
    <div className='p-6'>
      ...
      <div className='space-y-4'>
        ...
        <div className='mt-6 p-4 bg-cosmic-purple/10 border border-cosmic-purple/30 rounded-lg'>
          ...
          <p className='text-cosmic-text text-sm'>
            This channel creates a defined connection between centers,
            establishing consistent life force energy and reliable patterns in
            your Human Design. The {channelData.circuit.toLowerCase()} circuit
            carries the theme of {channelData.theme.toLowerCase()}.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className='bg-cosmic-dark border border-cosmic-purple/30 rounded-lg shadow-2xl shadow-cosmic-purple/20 max-w-2xl w-full max-h-[90vh] overflow-y-auto'>
        {data.type === 'gate'
          ? renderGateContent(data)
          : renderChannelContent(data)}
      </div>
    </div>
  );
};
