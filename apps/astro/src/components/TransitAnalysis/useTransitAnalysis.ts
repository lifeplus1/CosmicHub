...
export const useTransitAnalysis = (birthData: BirthData) => {
  ...
  const [dateRange, setDateRange] = useState<DateRange>(() => {
    ...
    const endSplit = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split('T');
    return {
  ...
  const calculateTransits = useCallback(
    async (options: TransitAnalysisOptions = {}) => {
      setLoading(true);
      setError(undefined);
      ...
      try {
        ...
        devConsole.log?.(
          '🚀 Calculating transits with payload:',
          requestPayload
        );
        ...
        if (Array.isArray(response.data) === true) {
          ...
          devConsole.log?.(
            `✅ Transit calculation successful: ${response.data.length} results`
          );
        } else {
        ...
        if (
          ...
          const detail = (err.response.data as Record<string, unknown>)?.[
            'detail'
          ];
          const message = (err.response.data as Record<string, unknown>)?.[
            'message'
          ];
          const errorMessage =
          ...
          setError(
            'Unable to connect to transit calculation service. Please check your connection.'
          );
        } else {
  ...
  const calculateLunarTransits = useCallback(
    async (options: LunarAnalysisOptions = {}) => {
      ...
      try {
        ...
        devConsole.log?.(
          '🌙 Calculating lunar transits with payload:',
          requestPayload
        );
        ...
        if (Array.isArray(response.data) === true) {
          ...
          devConsole.log?.(
            `✅ Lunar transit calculation successful: ${response.data.length} results`
          );
        } else {
        ...
        if (
          ...
          // Request was made but no response received
          setError(
            'Unable to connect to lunar transit service. Please check your connection.'
          );
        } else {
          // Something else happened
  ...
  const memoizedTransitResults = useMemo(
    () => transitResults,
    [transitResults]
  );
  ...
  const transitSummary = useMemo(() => {
    ...
    const majorAspects = transitResults.filter(t =>
      majorAspectSet.has(t.aspect)
    );
    const challengingAspects = transitResults.filter(t =>
      challengingSet.has(t.aspect)
    );
    ...
    return {
      ...
      intensity:
        transitResults.length > 0
          ? Math.round(
              transitResults.reduce((sum, t) => sum + (t.intensity ?? 0), 0) /
  ...
  const lunarSummary = useMemo(() => {
    const newMoons = lunarTransits.filter(l =>
      l.phase.toLowerCase().includes('new')
    );
    const fullMoons = lunarTransits.filter(l =>
      l.phase.toLowerCase().includes('full')
    );
    ...
    return {
      ...
      averageIntensity:
        lunarTransits.length > 0
          ? Math.round(
              lunarTransits.reduce((sum, l) => sum + (l.intensity ?? 0), 0) /
