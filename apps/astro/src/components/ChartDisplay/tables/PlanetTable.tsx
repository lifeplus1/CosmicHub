...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Tooltip } from '@cosmichub/ui';
import { getPlanetSymbol, getSignSymbol, getPlanetInterpretation } from './tableUtils';

export interface PlanetRow { name: string; sign: string; house: number; degree: string; position?: number; retrograde?: boolean }

const PlanetTable: React.FC<{ data: PlanetRow[] }> = ({ data }) => {
  if (!Array.isArray(data) || data.length === 0) return null;
  return (
    <Table aria-describedby="planet-table-caption">
      <caption id="planet-table-caption" className="sr-only">Table of planetary positions with sign, house and degree</caption>
      <TableHeader>
        <TableRow>
