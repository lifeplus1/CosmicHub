...
import { getRulerFromSign, getSignFromDegrees, calculateHousePosition } from '../../../utils/astrologyUtils';
import { AstrologySettings } from '../AstrologySettings';
import { getCelestialBodyCategory, CelestialBodyCategory } from '../../../utils/celestialBodyCategorization';
...
export interface CelestialBody {
  ...
  house: string;
  retrograde?: boolean;
  category: 'planets' | 'nodes' | 'major_asteroids' | 'minor_asteroids' | 'angles' | 'points';
}
...
const BODY_ORDER_WITHIN_CATEGORY: Record<string, number> = {
  ...
  // Nodes
  'North_node': 11, 'South_node': 12, 'North Node': 11, 'South Node': 12,
  // Major asteroids
  'Chiron': 13, 'Ceres': 14, 'Pallas': 15, 'Juno': 16, 'Vesta': 17,
  // Angles
  'Ascendant': 18, 'Midheaven': 19, 'Descendant': 20, 'IC': 21, 'Imumcoeli': 21,
  // Points
  'Vertex': 22, 'Antivertex': 23, 'Part of Fortune': 24,
};
...
const CATEGORY_MAPPING: Record<CelestialBodyCategory, CelestialBody['category']> = {
...
function formatDegree(degree: number | undefined, position: number | undefined): string {
  if (degree !== undefined) {
...
function CelestialBodiesTable({ bodies, showHouseRulers = false, settings }: CelestialBodiesTableProps) {
  ...
  const filteredBodies = bodies.filter(body => {
    ...
    switch (category) {
      case 'traditional_planets':
        console.log(`🔍 ${body.name}: traditionalPlanets = ${settings.celestialBodies.traditionalPlanets}`);
        return settings.celestialBodies.traditionalPlanets;
      case 'modern_planets':
        console.log(`🔍 ${body.name}: modernPlanets = ${settings.celestialBodies.modernPlanets}`);
        return settings.celestialBodies.modernPlanets;
      case 'major_asteroids':
        console.log(`🔍 ${body.name}: majorAsteroids = ${settings.celestialBodies.majorAsteroids}`);
        return settings.celestialBodies.majorAsteroids;
      case 'minor_asteroids':
        console.log(`🔍 ${body.name}: minorAsteroids = ${settings.celestialBodies.minorAsteroids}`);
        return settings.celestialBodies.minorAsteroids;
      case 'lunar_nodes':
        console.log(`🔍 ${body.name}: lunarNodes = ${settings.celestialBodies.lunarNodes}`);
        return settings.celestialBodies.lunarNodes;
      case 'lilith_points':
        console.log(`🔍 ${body.name}: lilithPoints = ${settings.celestialBodies.lilithPoints}`);
        return settings.celestialBodies.lilithPoints;
      case 'special_points':
        console.log(`🔍 ${body.name}: specialPoints = ${settings.celestialBodies.specialPoints}`);
        return settings.celestialBodies.specialPoints;
      case 'hypothetical':
        console.log(`🔍 ${body.name}: hypotheticalPoints = ${settings.celestialBodies.hypotheticalPoints}`);
      ...
      default:
        ...
        if (body.category === 'planets') return settings.celestialBodies.planets;
        if (body.category === 'major_asteroids' || body.category === 'minor_asteroids') return settings.celestialBodies.asteroids;
        if (body.category === 'points') return settings.celestialBodies.points;
        return true;
    }
  });
  
  console.log('🔍 CelestialBodiesTable: Filtered bodies count:', filteredBodies.length);
  ...
  const groupedBodies = sortedBodies.reduce((groups, body) => {
      ...
      {Object.entries(groupedBodies).map(([category, categoryBodies]) => (
        <div key={category} className="space-y-2">
          <h3 className="text-lg font-semibold text-cosmic-gold border-b border-cosmic-gold/30 pb-1">
            {CATEGORY_LABELS[category as keyof typeof CATEGORY_LABELS] || `${category.charAt(0).toUpperCase() + category.slice(1)}`}
          </h3>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {(!settings || settings.displayOptions.showDegrees) && <TableHead>Degree</TableHead>}
                <TableHead>House</TableHead>
                {showHouseRulers && category === 'angles' && <TableHead>Ruler</TableHead>}
                {category !== 'angles' && (!settings || settings.displayOptions.showRetrograde) && <TableHead>Status</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
                  ...
                  <TableRow key={`${category}-${body.name}-${index}`}>
                    ...
                    {showHouseRulers && category === 'angles' ? (
                      <TableCell>
                        {(() => {
                          try {
                            const ruler = getRulerFromSign(body.sign.toLowerCase() as any);
                    ...
                    ) : (category !== 'angles' && (!settings || settings.displayOptions.showRetrograde)) ? (
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`text-xs ${
                            body.retrograde ? "text-cosmic-red" : "text-cosmic-silver opacity-60"
                          }`}>
...
export function convertToCelestialBodies(chartData: {
  ...
  if (chartData.angles) {
    ...
    if (chartData.angles.antivertex !== undefined) {
      angleEntries.push({ name: 'antivertex', position: chartData.angles.antivertex });
    }
    if (chartData.angles.part_of_fortune !== undefined) {
      angleEntries.push({ name: 'part_of_fortune', position: chartData.angles.part_of_fortune });
    }
    ...
    angleEntries.forEach(angle => {
      if (typeof angle.position === 'number') {
        bodies.push({
          ...
          house: calculateHousePosition(angle.position, chartData.houses?.map(h => h.cusp) || []).toString(),
          position: angle.position,
          degree: angle.position % 30, // Position within sign
          category: angle.name === 'vertex' || angle.name === 'antivertex' || angle.name === 'part_of_fortune' ? 'points' : 'angles',
        });
      }
    });
  }
