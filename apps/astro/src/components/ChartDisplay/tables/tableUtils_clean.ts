...
export const getCelestialBodyCategory = (name: string): string => {
  ...
  if (['sun', 'moon', 'mercury', 'venus', 'mars', 'jupiter', 'saturn'].includes(lowerName)) {
    return 'traditional';
  }
  ...
  if (['chiron', 'ceres', 'pallas', 'juno', 'vesta', 'psyche'].includes(lowerName)) {
    return 'major_asteroids';
  }
  
  // Minor asteroids
  if (['eros', 'fortuna', 'sedna', 'eris', 'hygiea', 'astraea', 'hebe', 'iris', 'flora', 'metis'].includes(lowerName)) {
    return 'minor_asteroids';
  }
  
  // Lunar nodes
  if (['north_node', 'south_node'].includes(lowerName) || name.includes('node')) {
    return 'lunar_nodes';
  }
  ...
  // Special points
  if (['vertex', 'antivertex', 'part of fortune', 'part_of_fortune'].includes(lowerName) || name.includes('Fortune')) {
    return 'special_points';
  }
  
  // Hypothetical points (Uranian/Hamburg School)
  if (['cupido', 'hades', 'zeus', 'kronos', 'apollon', 'admetos', 'vulkanus', 'poseidon'].includes(lowerName) || 
      name.includes('intp_')) {
    return 'hypothetical_points';
  }
  
  // Angles
  if (['ascendant', 'descendant', 'midheaven', 'ic', 'imumcoeli'].includes(lowerName) || 
      name.includes('Coeli')) {
    return 'angles';
  }
