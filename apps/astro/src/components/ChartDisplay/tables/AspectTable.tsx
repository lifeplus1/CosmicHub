...
export interface AspectRow { planet1: string; planet2: string; type: string; orb: string; applying: string }

const AspectTable: React.FC<{ data: AspectRow[] }> = ({ data }) => {
  if (!Array.isArray(data) || data.length === 0) return null;
  return (
    <Table aria-describedby="aspect-table-caption">
      <caption id="aspect-table-caption" className="sr-only">Table of planetary aspects including aspect type, orb and applying/separating status</caption>
      <TableHeader>
        <TableRow>
