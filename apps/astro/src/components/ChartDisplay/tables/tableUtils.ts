...
export const getPlanetSymbol = (name: string): string => {
  console.log('getPlanetSymbol called with:', name, 'PLANET_SYMBOLS[name]:', PLANET_SYMBOLS[name]);
...
export const getSignSymbol = (sign: string): string => {
  console.log('getSignSymbol called with:', sign, 'SIGN_SYMBOLS[sign]:', SIGN_SYMBOLS[sign]);
...
export const getAspectSymbol = (aspect: string): string => {
  console.log('getAspectSymbol called with:', aspect, 'ASPECT_SYMBOLS[aspect]:', ASPECT_SYMBOLS[aspect]);
...
export const getPlanetInterpretation: PlanetInterpretationFn = (planet, sign) => {
  if (planet === 'Moon') {
    return MOON_SIGN_INTERPRETATIONS[sign] ?? SIGN_INTERPRETATIONS[sign] ?? 'Emotional influence';
  }
