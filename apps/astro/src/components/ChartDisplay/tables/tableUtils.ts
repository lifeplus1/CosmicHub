...
const INTERPRETATIONS: Record<string, Record<string, string>> = {
  Sun: {
    Aries: 'Bold leadership drive',
    Taurus: 'Grounded creative force',
    Gemini: 'Curious communicator',
    Cancer: 'Protective and nurturing',
    Leo: 'Radiant self-expression',
    Virgo: 'Analytical purpose',
    Libra: 'Seeks relational harmony',
    Scorpio: 'Intense transformative will',
    Sagittarius: 'Adventurous seeker',
    Capricorn: 'Ambitious achiever',
    Aquarius: 'Innovative visionary',
    Pisces: 'Compassionate dreamer',
  },
  Moon: {
    Aries: 'Fast, reactive emotions',
    Taurus: 'Comfort in stability',
    Gemini: 'Changeable mental moods',
    Cancer: 'Deep emotional attunement',
    Leo: 'Expressive heart-centered feelings',
    Virgo: 'Practical emotional processing',
    Libra: 'Needs balance & partnership',
    Scorpio: 'Intense emotional depth',
    Sagittarius: 'Restless emotional freedom',
    Capricorn: 'Reserved emotional control',
    Aquarius: 'Detached intuitive insight',
...
export const getPlanetInterpretation: PlanetInterpretationFn = (
  ...
  if (
    planetMap?.[interpSign] !== undefined &&
    planetMap[interpSign].length > 0
  ) {
...
export const aspectStatusClass = (status: string): string => {
  if (typeof status === 'string' && status === 'Exact')
    return 'text-cosmic-gold';
  if (typeof status === 'string' && status === 'Applying')
