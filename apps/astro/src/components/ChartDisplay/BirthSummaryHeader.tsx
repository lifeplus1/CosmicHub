...
export const BirthSummaryHeader: React.FC<Props> = ({ birthData, isLoading, onEdit, onRecalculate, onSave }) => {
  const { year, month, day, hour, minute, city, latitude: lat, longitude: lon } = birthData;

  // Keyboard shortcuts (e to edit, r to recalc, s to save) when focus is inside region
  const onKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.altKey || e.metaKey || e.ctrlKey) return;
    if (e.key === 'e') { onEdit(); }
    else if (e.key === 'r') { if (!isLoading) onRecalculate(); }
    ...
    <Card className='p-6' aria-describedby='birth-data-summary' role='region' aria-labelledby='chart-page-heading' tabIndex={-1} onKeyDown={onKeyDown}>
      <div className='flex flex-col md:flex-row md:justify-between gap-4 items-start'>
        <div>
          ...
          <div id='birth-data-summary' className='text-sm text-gray-600 space-y-1'>
            <p>
              ...
              <time dateTime={`${dateStr}T${timeStr}:00Z`} suppressHydrationWarning>{dateStr}</time>{' '}
              at <time dateTime={`${dateStr}T${timeStr}:00Z`} suppressHydrationWarning>{timeStr}</time>
            </p>
            <p>
            ...
            {typeof lat === 'number' && typeof lon === 'number' && (
              <p>
                ...
                <span aria-label={`Latitude ${lat.toFixed(4)} degrees, Longitude ${lon.toFixed(4)} degrees`}>
        ...
        <div className='flex flex-wrap gap-2' aria-label='Chart actions'>
          <Button variant='secondary' onClick={onEdit} aria-label='Edit birth data (e)'>
            Edit
          </Button>
          <Button
            ...
            disabled={isLoading}
            aria-live='polite'
            aria-label={isLoading ? 'Chart calculation in progress' : 'Recalculate chart (r)'}
          >
