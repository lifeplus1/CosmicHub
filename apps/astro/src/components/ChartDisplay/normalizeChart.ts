...
export interface ChartLike { planets?: unknown; houses?: unknown; aspects?: unknown; asteroids?: unknown; angles?: unknown }

interface LoosePlanetInput { name?: string; sign?: string; position?: number; degree?: number; house?: string | number; aspects?: unknown[]; retrograde?: boolean }
interface LooseHouseInput { house?: number; number?: number; sign?: string; degree?: number; cusp?: number; ruler?: string }
interface LooseAspectInput { planet1?: string; planet2?: string; type?: string; orb?: number | string; applying?: string }
interface LooseAsteroidInput { name?: string; sign?: string; degree?: number; house?: string | number }
interface LooseAngleInput { name?: string; sign?: string; degree?: number; position?: number | string }

export function isChartLike(obj: unknown): obj is ChartLike {
  if (obj === null || typeof obj !== 'object') return false;
  const c = obj as Record<string, unknown>;
  return 'planets' in c || 'houses' in c || 'aspects' in c || 'asteroids' in c || 'angles' in c;
}

export function hasChartContent(chart: ChartLike): boolean {
  return [chart.planets, chart.houses, chart.aspects, chart.asteroids, chart.angles].some(section => {
...
export const getRulerFromSign = (sign: string): string => {
  const rulers: Record<string, string> = { Aries:'Mars', Taurus:'Venus', Gemini:'Mercury', Cancer:'Moon', Leo:'Sun', Virgo:'Mercury', Libra:'Venus', Scorpio:'Mars', Sagittarius:'Jupiter', Capricorn:'Saturn', Aquarius:'Uranus', Pisces:'Neptune' };
  const value = rulers[sign];
  return (typeof value === 'string' && value.length > 0) ? value : 'Unknown';
};

export const getAspectOrb = (aspectType: string, currentOrb?: number): number => {
  const aspectTypeLower = typeof aspectType === 'string' ? aspectType.toLowerCase() : '';
  if (currentOrb !== null && currentOrb !== undefined && Number.isNaN(currentOrb) === false) return currentOrb;
  if (aspectTypeLower.includes('conjunction') || aspectTypeLower.includes('opposition')) return 10;
  return 8;
};
...
function toPlanetArray(input: unknown): ChartDisplayPlanet[] {
  ...
  if (typeof input === 'object') {
    return Object.entries(input as Record<string, LoosePlanetInput>).map(([name, data]) => {
  const pos = (typeof data.position === 'number' ? data.position : (typeof data.degree === 'number' ? data.degree : 0));
      ...
      return {
        ...
        house: typeof data.house === 'number' ? String(data.house) : (data.house ?? 'Unknown'),
...
function toHouseArray(input: unknown): ChartDisplayHouse[] {
  ...
  const cusp = typeof h.cusp === 'number' ? h.cusp : (typeof h.degree === 'number' ? h.degree : 0);
    return {
  ...
  if (typeof input === 'object') return Object.values(input as Record<string, LooseHouseInput>).map(mapOne);
  return [];
}
...
function toAspectArray(input: unknown): ChartDisplayAspect[] {
  ...
  const rawOrb = typeof a.orb === 'number' ? a.orb : (typeof a.orb === 'string' ? (Number.isNaN(parseFloat(a.orb)) ? 0 : parseFloat(a.orb)) : 0);
...
function toAngleArray(input: unknown): ChartDisplayAngle[] {
  ...
  if (input !== null && typeof input === 'object') {
    return Object.entries(input as Record<string, number | string | LooseAngleInput>).map(([name, val]) => {
      let position: number = 0;
      if (typeof val === 'number') position = val;
  else if (typeof val === 'string') position = (Number.isNaN(parseFloat(val)) ? 0 : parseFloat(val));
      else if (typeof val === 'object') position = typeof val.position === 'number' ? val.position : (typeof val.degree === 'number' ? val.degree : 0);
