...
export interface EnhancedAspectRow {
  planet1: string; planet2: string; aspect: string; aspectType: AspectType; orb: number; strength: 'exact'|'strong'|'moderate'|'weak'; applying: boolean; isMajor: boolean; angularDifference: number;
}

export function useEnhancedAspects(aspects: ChartDisplayAspect[]) {
  return useMemo<EnhancedAspectRow[]>(() => aspects.map(a => {
    ...
    return {
      ...
      orb: a.orb,
      strength: absOrb < 1 ? 'exact' : absOrb < 2 ? 'strong' : absOrb < 4 ? 'moderate' : 'weak',
      applying,
      isMajor: ['conjunction','opposition','trine','square','sextile'].includes(a.type.toLowerCase()),
