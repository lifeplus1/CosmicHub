...
export type CelestialPointCategory =
  | 'lunar_nodes'
  | 'lilith_points'
  | 'special_points'
  | 'hypothetical';
...
export function useCategorizedPoints(points: ChartDisplayPlanet[]) {
  return useMemo<CategorizedPointsBuckets>(() => {
    ...
    points.forEach(p => {
      const cat = getCelestialBodyCategory(p.name) as
        | CelestialPointCategory
        | undefined;
      if (cat && buckets[cat]) buckets[cat].push(p);
    });
    if (process.env.NODE_ENV === 'development') {
      (window as unknown as Record<string, unknown>).__categorizedPoints =
        buckets;
    }
    return buckets;
  }, [points]);
