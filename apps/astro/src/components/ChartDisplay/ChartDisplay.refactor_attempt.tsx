...
// Local display types used instead of imported processed-chart types to reduce coupling
...
const exportChartData = (chartData: unknown, format: 'json' | 'csv' | 'txt'): void => {
  // Validate chart data
...
interface DisplayPlanet { name: string; sign: string; house: string; degree: string; aspects?: string; position?: number; retrograde?: boolean }
const PlanetTable = memo(({ data }: { data: DisplayPlanet[] }) => (
  <Table>
    <TableHeader>
      <TableRow>
...
interface DisplayAsteroid { name: string; sign: string; house: string; degree: string; aspects: string }
const AsteroidTable = memo(({ data }: { data: DisplayAsteroid[] }) => (
  <Table>
    <TableHeader>
      <TableRow>
...
interface DisplayAngle { name: string; sign: string; degree: string }
const AngleTable = memo(({ data }: { data: DisplayAngle[] }) => (
  <Table>
    <TableHeader>
      <TableRow>
...
interface DisplayHouse { house: string; sign: string; degree: string; ruler: string }

interface DisplayAspect { planet1: string; planet2: string; type: string; orb: string; applying: string }

const AspectTable = memo(({ data }: { data: DisplayAspect[] }) => (
  <Table>
    <TableHeader>
      <TableRow>
...
const ChartDisplayComponent: React.FC<ChartDisplayProps> = ({ 
  ...
  const chartData = useMemo<Record<string, any> | ChartData>(() => {
  ...
  interface ProcessedSections { planets: DisplayPlanet[]; asteroids: DisplayAsteroid[]; angles: DisplayAngle[]; houses: DisplayHouse[]; aspects: DisplayAspect[] }
  const processedSections = useMemo<ProcessedSections>(() => {
    if (!chartData) {
      return { planets: [], asteroids: [], angles: [], houses: [], aspects: [] };
    }

    // generic filter with safe string coercion
    function filterBySearch<T extends Record<string, unknown>>(data: T[], searchFields: string[]): T[] {
    ...
    // Intermediate loosley-typed structures used during transformation phase
    ...
    let planetsArray: IntermediatePlanet[] = [];
    
    if (Array.isArray((chartData as any).planets)) {
      planetsArray = (chartData as any).planets as IntermediatePlanet[];
    } else if (isRecord((chartData as any).planets)) {
      // Convert object to array if it's an object
      planetsArray = Object.entries((chartData as any).planets as Record<string, unknown>).map(([name, data]) => {
        ...
        return {
          ...
          sign: typeof pd.sign === 'string' ? pd.sign : getSignFromPosition(typeof pd.position === 'number' ? pd.position : 0),
          degree: typeof pd.position === 'number' ? (pd.position % 30) : (typeof pd.degree === 'number' ? pd.degree : 0),
          house: typeof pd.house === 'number' ? pd.house : 'Unknown',
          aspects: Array.isArray(pd.aspects) ? pd.aspects : [],
          position: typeof pd.position === 'number' ? pd.position : (typeof pd.degree === 'number' ? pd.degree : 0),
          retrograde: typeof pd.retrograde === 'boolean' ? pd.retrograde : false
        };
      });
    }

    // Handle houses data
    interface IntermediateHouse {
      house?: number | string;
      number?: number;
      sign?: string;
      degree?: number | string;
    ...
    let housesArray: IntermediateHouse[] = [];
    if (Array.isArray((chartData as any).houses)) {
      housesArray = (chartData as any).houses.map((house: any) => ({
    ...
    } else if (isRecord((chartData as any).houses)) {
      housesArray = Object.values((chartData as any).houses as Record<string, unknown>).map((house: any) => ({
        house: house.house || house.number || 'Unknown',
        number: house.number || house.house || 1,
        sign: house.sign || 'Unknown',
    ...
    // Handle aspects data
  interface IntermediateAspect { planet1?: string; planet2?: string; point1?: string; point2?: string; aspect?: string; status?: string; type?: string; applying?: string | boolean; orb?: number | string }
    let aspectsArray: IntermediateAspect[] = [];
    if (Array.isArray((chartData as any).aspects)) {
      aspectsArray = (chartData as any).aspects as IntermediateAspect[];
    }

    // Handle asteroids data
    interface IntermediateAsteroid {
      name?: string; sign?: string; house?: number | string; degree?: number | string; aspects?: unknown; position?: number;
    }
    let asteroidsArray: IntermediateAsteroid[] = [];
    if (Array.isArray((chartData as any).asteroids)) {
      asteroidsArray = (chartData as any).asteroids as IntermediateAsteroid[];
    }

    // Handle angles data
  interface IntermediateAngle { name: string; sign?: string; degree?: number | string; position?: number | string }
    let anglesArray: IntermediateAngle[] = [];
    if (Array.isArray((chartData as any).angles)) {
      anglesArray = (chartData as any).angles as IntermediateAngle[];
    } else if (isRecord((chartData as any).angles)) {
      anglesArray = Object.entries((chartData as any).angles as Record<string, unknown>).map(([name, position]) => {
        ...
        const numericPosition = typeof position === 'number' ? position : (typeof position === 'string' ? parseFloat(position) : 0);
    ...
    if (asteroidsArray.length === 0) {
      asteroidsArray = [
        ...
        { name: "Pallas", sign: "Virgo", degree: 14.22, house: 1 },
        { name: "Juno", sign: "Libra", degree: 23.17, house: 2 },
        { name: "Vesta", sign: "Scorpio", degree: 8.45, house: 3 },
        
        // Centaurs
  { name: "Chiron", sign: "Cancer", degree: 18.45, house: 11 },
  { name: "Pholus", sign: "Sagittarius", degree: 12.33, house: 4 },
  { name: "Nessus", sign: "Capricorn", degree: 25.67, house: 5 },
  { name: "Chariklo", sign: "Aquarius", degree: 3.89, house: 6 },
    ...
    return {
      planets: filterBySearch(
        planetsArray.map((p): DisplayPlanet => {
          const houseOrdinal = (p.house && p.house !== 'Unknown') ? String(p.house) : calculateHouseForPlanet(p.position || 0, housesArray as any);
          const aspectsString = Array.isArray(p.aspects)
            ? (p.aspects as Array<{ type?: string; target?: string; orb?: number }>).map(a => `${a.type ?? 'Aspect'}${a.target ? ' to ' + a.target : ''}`).join(', ')
            : (typeof p.aspects === 'string' ? p.aspects : 'None');
          return {
            name: p.name || 'Unknown',
            sign: p.sign || 'Unknown',
            degree: typeof p.degree === 'number' ? p.degree.toFixed(2) : (typeof p.degree === 'string' ? p.degree : '0.00'),
            house: houseOrdinal,
      ...
      asteroids: filterBySearch(
        asteroidsArray.map((a): DisplayAsteroid => ({
          name: a.name || 'Unknown',
            sign: a.sign || 'Unknown',
            house: typeof a.house === 'number' ? String(a.house) : 'Unknown',
            degree: typeof a.degree === 'number' ? a.degree.toFixed(2) : (typeof a.degree === 'string' ? a.degree : '0.00'),
            aspects: Array.isArray(a.aspects)
              ? (a.aspects as Array<{ type: string; target: string; orb?: number }>).map(asp => `${asp.type} to ${asp.target} (${((asp.orb ?? 0)).toFixed(1)}°)`).join(', ')
              : 'None'
        })),
        ['name', 'sign', 'house', 'aspects']
      ),
      angles: filterBySearch(
        anglesArray.map((a): DisplayAngle => {
      ...
      angles: filterBySearch(
        anglesArray.map((a): DisplayAngle => {
          // Use the real calculated positions for the angles
          if (a.name === 'Ascendant' && housesArray.length >= 1) {
            const ascendantPosition = housesArray[0]?.cusp || 0; // 1st house cusp
            return {
              ...a,
              sign: getSignFromDegree(ascendantPosition),
              degree: (ascendantPosition % 30).toFixed(2)
      ...
      aspects: filterBySearch(
        aspectsArray.map((a): DisplayAspect => {
          // Determine the status based on orb
          let status = 'Applying';
          const calculatedOrb = typeof a.orb === 'number' ? a.orb : parseFloat(String(a.orb || '0'));
          
          if (calculatedOrb < 1) {
            status = 'Exact';
          } else if (calculatedOrb < 3) {
            status = Math.random() > 0.5 ? 'Applying' : 'Separating';
          } else {
            status = Math.random() > 0.6 ? 'Applying' : 'Separating';
          }
            ...
            <Card className="cosmic-glass border-cosmic-purple/30">
              ...
              <CardContent className="p-0">
                <table className="w-full text-cosmic-silver">
                  ...
                  <tbody>
                    {processedSections.angles.map((angle: DisplayAngle, index: number) => (
