...
describe('BirthSummaryHeader accessibility & shortcuts', () => {
  ...
  it('announces recalculation state changes', async () => {
    ...
    await waitFor(() => {
      ...
      console.log('Button disabled:', calculatingButton?.hasAttribute('disabled'));
      console.log('Button aria-disabled:', calculatingButton?.getAttribute('aria-disabled'));
  ...
  it('supports keyboard shortcuts (e, r, s)', () => {
    ...
    const region = container.querySelector('[role="region"][aria-labelledby="chart-page-heading"]') as HTMLElement;
