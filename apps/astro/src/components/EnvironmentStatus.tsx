...
export const EnvironmentStatus: React.FC<EnvironmentStatusProps> = ({
  ...
  const isFirebaseConfigured = Object.values(firebaseConfig).every(value => {
    return (
      typeof value === 'string' &&
      value.length > 0 &&
      value.includes('placeholder') === false
    );
  });
  ...
  return (
    <div className={`rounded-xl border p-6 ${className}`}>
      <div className='flex items-center mb-4'>
        ...
        <h3 className='text-lg font-semibold text-cosmic-gold font-playfair'>
          Environment Status
        </h3>
      </div>
      ...
      <div className='space-y-3'>
        <div className='flex items-center justify-between p-3 bg-cosmic-dark/30 rounded-lg'>
          <span className='text-sm text-cosmic-silver/80 font-medium'>
            Mode:
          </span>
          <span
          ...
          <div className='flex items-start'>
            ...
            <div>
              ...
              <p className='text-xs text-orange-200/80 leading-relaxed'>
                Using placeholder Firebase credentials. Set real Firebase
                environment variables for full authentication functionality.
              </p>
            </div>
          </div>
        </div>
