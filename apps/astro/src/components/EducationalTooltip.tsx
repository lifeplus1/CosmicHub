...
export const EducationalTooltip = React.memo(
  React.forwardRef<HTMLDivElement, EducationalTooltipProps>(
    ({
      title,
      description,
      examples = [],
        ...
        <div className='flex flex-col space-y-2 max-w-[300px] p-2'>
          <div className='flex justify-between w-full'>
            ...
            {tier !== null && tier !== undefined && (
              <span
                className={`bg-${getTierColor(tier)}/20 text-${getTierColor(tier)} px-2 py-1 rounded text-xs`}
              >
          ...
          {Array.isArray(examples) && examples.length > 0 && (
            <div className='flex flex-col space-y-1'>
              ...
              {examples.map((example, index) => (
                <p key={index} className='text-xs text-cosmic-silver/60'>
                  • {example}
                </p>
              ))}
            </div>
      ...
      return (
        <Tooltip.Root>
          <Tooltip.Trigger
            asChild={children !== null && children !== undefined}
          >
            {children ?? (
              <button
