    ...
    async (e: React.FormEvent): Promise<void> => {
      ...
      try {
        ...
        const response = await axios.post(
          `${apiConfig.baseUrl}/analyze-personality`,
          {
        ...
        if (
          ...
          'sun_sign' in responseData &&
          'traits' in responseData
        ) {
        ...
        const errorMessage =
          err.response?.data?.detail ?? 'Failed to analyze personality';
      ...
      <form onSubmit={onFormSubmit} aria-label='Personality Analysis Form'>
        <div className='flex flex-col space-y-4'>
          ...
          <div>
            ...
            >
              House System <span aria-hidden='true'>*</span>
            </label>
            <select
              id='houseSystem'
              name='houseSystem'
          ...
          {result !== null && (
            <div className='p-4 mt-4 rounded-lg bg-cosmic-blue/30'>
              <p className='text-cosmic-silver'>
                Sun Sign: <span className='font-bold'>{result.sun_sign}</span>
              </p>
              <p className='text-cosmic-silver'>
                Traits: <span className='font-bold'>{result.traits}</span>
              </p>
            </div>
          )}
        </div>
