...
const PricingPage: React.FC = React.memo(() => {
  ...
  const getTierIcon = (tier: keyof typeof COSMICHUB_TIERS) => {
    switch (tier) {
      ...
      case 'elite':
        return <FaCrown className='text-orange-500' />;
      default:
        return <FaUser className='text-gray-500' />;
    }
  };
  ...
  const getFeatureIcon = (feature: string) => {
    switch (feature) {
      ...
      case 'Western tropical astrology':
        return <FaUsers className='text-green-500' />;
      case 'Multi-system analysis':
        return <FaBrain className='text-purple-500' />;
      case 'Vedic sidereal charts':
        return <FaMagic className='text-orange-500' />;
      case 'Chinese astrology':
        return <FaInfinity className='text-red-500' />;
      case 'Mayan calendar':
        return <FaStar className='text-yellow-500' />;
      case 'Uranian astrology':
        return <FaCrown className='text-gold-500' />;
      case 'Synastry compatibility':
        return <FaHeart className='text-pink-500' />;
      case 'AI interpretations':
        return <FaBrain className='text-indigo-500' />;
      case 'Transit analysis':
        return <FaCalendarAlt className='text-cyan-500' />;
      case 'PDF export':
        return <FaFilePdf className='text-red-500' />;
      case 'Unlimited calculations':
        return <FaInfinity className='text-green-500' />;
      case 'Unlimited storage':
        return <FaSave className='text-blue-500' />;
  ...
  const getFeatureDescription = (feature: string): string => {
    const descriptions: Record<string, string> = {
      'AI interpretations':
        'Intelligent chart analysis powered by advanced AI models.',
      'Transit analysis':
        'Detailed forecasts of upcoming planetary influences and timing.',
      'Multi-system analysis':
        'Integrated Western, Vedic, Chinese, Mayan, and Uranian astrology.',
      'Synastry compatibility':
        'In-depth relationship analysis between multiple charts.',
      'Vedic sidereal charts':
        'Traditional Indian astrology with sidereal zodiac.',
    ...
    return desc === undefined || desc === ''
      ? 'Advanced astrological feature for deeper insights.'
      : desc;
  };
  ...
  const getFeatureTier = (
    feature: string
  ): 'free' | 'premium' | 'elite' | undefined => {
    ...
    <div className='min-h-screen px-4 py-12 bg-gray-50'>
      <div className='mx-auto max-w-7xl'>
        <div className='mb-12 space-y-4 text-center'>
          <h2 className='text-4xl font-bold text-cosmic-gold font-cinzel'>
            Pricing Plans
          </h2>
          <p className='max-w-2xl mx-auto text-lg text-cosmic-silver'>
            Choose the perfect plan to unlock the cosmos. All plans include core
            astrology features, with premium tiers adding advanced analysis and
            unlimited access.
          </p>
        </div>
              ...
              >
                <div className='p-6'>
                  ...
                  <div className='flex items-baseline mb-6 space-x-2'>
                    ...
                    <span className='text-sm text-cosmic-silver/80'>
                      /{isAnnual ? 'year' : 'month'}
                    </span>
                  </div>
                            ...
                            <Tooltip.Root>
                              ...
                              <Tooltip.Portal>
                                ...
                                >
                                  ...
                                  <div className='flex flex-col space-y-1'>
                                    {getFeatureExamples(feature).map(
                                      (example, i) => (
                                        <p
                                          key={i}
                                          className='text-xs text-cosmic-silver/80'
                                        >
                                          • {example}
                                        </p>
                                      )
                  ...
                  {tier.price.monthly === 0 &&
                    typeof tier.price.monthly === 'number' && (
                      <p className='mt-2 text-xs text-center text-cosmic-silver/80'>
                        Always free
                      </p>
                    )}
                </div>
        ...
        <div className='space-y-4 text-center'>
          ...
          <p className='text-sm text-cosmic-silver'>
            Hover over features for more details
          </p>
        </div>
        ...
        <div className='mt-8 overflow-x-auto'>
          <table className='w-full table-auto'>
            <thead>
              <tr>
                <th className='px-4 py-3 font-semibold text-left text-cosmic-silver'>
                  Feature
                </th>
                <th className='px-4 py-3 font-semibold text-center text-cosmic-silver'>
                  Free
                </th>
                <th className='px-4 py-3 font-semibold text-center text-cosmic-silver'>
                  Premium
