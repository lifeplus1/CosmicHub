              ...
              <Accordion.Trigger className='flex justify-between w-full p-4 transition-colors duration-300 bg-purple-500/20 hover:bg-purple-500/30 lg:p-6'>
                <div className='flex space-x-2'>
                  ...
                  <span className='px-2 py-1 text-sm text-cosmic-silver rounded bg-cosmic-purple'>
                    {Object.keys(data.planets).length}
                  </span>
                </div>
                <ChevronDownIcon />
              </Accordion.Trigger>
                  ...
                  <table className='w-full table-auto'>
                    ...
                    <tbody>
                      {Object.entries(data.planets).map(
                        ([planet, info]: [
                          string,
                          { position: number; retrograde?: boolean },
                        ]) => (
                          <tr key={planet}>
                        ...
                        <tr key={idx}>
                          <td className='px-4 py-2 border-b border-cosmic-gold/20'>
                            <div className='flex space-x-2'>
                              <span className='text-lg'>
                                {aspectSymbols[
                                  aspect.aspect as keyof typeof aspectSymbols
                                ] || '—'}
                              </span>
                          ...
                          <td className='px-4 py-2 border-b border-cosmic-gold/20'>
                            <span className='text-cosmic-silver'>
                              {aspect.point1} - {aspect.point2}
                            </span>
                            <p className='text-sm text-cosmic-silver/70'>
                              {aspect.point1_sign} (H{aspect.point1_house}) -{' '}
                              {aspect.point2_sign} (H{aspect.point2_house})
                            </p>
                          </td>
                          <td className='px-4 py-2 border-b border-cosmic-gold/20'>
                            <span
                              className={`${aspect.exact === true ? 'bg-green-500 text-cosmic-dark' : 'bg-cosmic-purple/20 text-cosmic-purple'} px-2 py-1 rounded text-sm`}
                            >
