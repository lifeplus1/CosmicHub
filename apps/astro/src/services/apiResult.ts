...
export const fail = (error: string, code?: string): Failure => ({
  success: false,
  error,
  code,
});
...
export const toFailure = (error: unknown, opts: FailureMapOpts): Failure => {
  if (axios.isAxiosError(error)) {
    ...
    if (status === 401)
      return fail(opts.auth ?? 'Authentication required', '401'); // ALLOW_FAIL_USAGE - local helper
    if (status === 404)
      return fail(opts.notFound ?? 'Resource not found', '404'); // ALLOW_FAIL_USAGE - local helper
    if (status === 400)
      return fail(opts.validation ?? 'Validation error', '400'); // ALLOW_FAIL_USAGE - local helper
  }
