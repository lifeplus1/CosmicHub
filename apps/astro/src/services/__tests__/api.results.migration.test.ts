...
vi.mock('../../config/environment', () => ({
  devConsole: { log: vi.fn(), error: vi.fn(), warn: vi.fn() },
}));
vi.mock('@cosmichub/config/firebase', () => ({
  auth: {
...
const buildResponse = <T>(data: T, status = 200) => ({
  data,
  status,
  statusText: 'OK',
});
...
describe('API migration to ApiResult', () => {
  ...
  it('calculateHumanDesign returns success', async () => {
    mockedAxios.post.mockResolvedValue(
      buildResponse({ human_design: { type: 'Generator' } })
    );
    const result = await api.calculateHumanDesign({
      year: 2000,
      month: 1,
      day: 1,
      hour: 0,
  ...
  it('fetchAIInterpretations returns success', async (): Promise<void> => {
    ...
    const result = await api.fetchAIInterpretations(
      'chart-1' as unknown as api.ChartId,
      'user-1' as unknown as api.UserId
    );
    expect(result.success).toBe(true);
