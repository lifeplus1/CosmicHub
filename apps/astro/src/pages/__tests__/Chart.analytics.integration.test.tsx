...
vi.mock('../../components/ChartDisplay/ChartDisplay', () => ({
  default: () => <div data-testid='chart-display' />,
}));
...
vi.mock('../../services/analytics', () => ({
  trackCosmicHubChartCalculation: (data: any) =>
    trackCosmicHubChartCalculation(data),
...
describe('Chart page analytics integration', () => {
  ...
  it('fires api_error analytics on backend-reported failure and no view event', async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue({ success: false, error: 'backend failure' });
  ...
  it('does not call fetch or analytics when URL params incomplete (parseBirthParams returns null)', async () => {
    const fetchFn = vi.fn();
    render(
      <MemoryRouter initialEntries={['/chart?year=2000&month=1&day=2']}>
        {' '}
        {/* missing hour/minute etc */}
        <BirthDataProvider>
          <Chart fetchFn={fetchFn} />
        </BirthDataProvider>
      </MemoryRouter>
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole('heading', { name: /No Birth Data Available/i })
      ).toBeInTheDocument();
    });
