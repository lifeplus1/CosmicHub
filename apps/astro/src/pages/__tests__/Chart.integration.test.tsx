...
describe('Chart page integration (canonical pipeline)', () => {
  ...
  it('parses URL params into canonical fetch call', async () => {
    ...
    render(
      <MemoryRouter
        initialEntries={[
          '/chart?year=2001&month=1&day=2&hour=3&minute=4&city=Z&lat=1&lon=2&timezone=UTC',
        ]}
      >
        <BirthDataProvider>
          <Chart fetchFn={fetchFn} />
  ...
  it('handles timezone param propagation', async () => {
    ...
    render(
      <MemoryRouter
        initialEntries={[
          '/chart?year=2020&month=7&day=4&hour=10&minute=30&city=NYC&lat=40.7&lon=-74&timezone=America/New_York',
        ]}
      >
        <BirthDataProvider>
          <Chart fetchFn={fetchFn} />
