...
const Chart: React.FC = () => {
  ...
  // Set default birth data once on mount to prevent infinite loading
  React.useEffect(() => {
    if (!birthData) {
      const defaultData = {
        year: 1990,
      ...
      setBirthData(defaultData);
    }
  }, []); // Empty dependency array to run only once

  // Create stable query key
  const queryKey = React.useMemo(() => {
    if (!birthData) return ['chartData', 'no-data'];
    return ['chartData', birthData.year, birthData.month, birthData.day, birthData.hour, birthData.minute, birthData.city];
  }, [birthData]);
