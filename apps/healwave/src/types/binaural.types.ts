...
export type BinauralColor = 'purple' | 'blue' | 'green' | 'yellow' | 'red' | 'gray';
...
export type BinauralSettingsError = 
  | { type: 'INVALID_FREQUENCY'; frequency: number; constraints: FrequencyConstraints['baseFrequency'] }
  | { type: 'INVALID_BEAT'; beat: number; constraints: FrequencyConstraints['binauralBeat'] }
  | { type: 'INVALID_VOLUME'; volume: number; constraints: AudioSettingsConstraints['volume'] }
...
export interface BinauralAnalyticsEvent {
  readonly event: 'preset_created' | 'range_selected' | 'advanced_mode_toggled' | 'custom_frequency_used';
...
export interface InputValidation {
  ...
  readonly validateNumeric: (value: number, constraints: { min: number; max: number }) => boolean;
...
export const BINAURAL_RANGES: Readonly<Record<BinauralRangeKey, BinauralRange>> = {
  delta: { 
    min: 0.5, 
    max: 4, 
    ...
    benefits: ['Deep restorative sleep', 'Growth hormone release', 'Immune system boost']
  },
  theta: { 
    min: 4, 
    max: 8, 
