...
function saveBaseline(count) {
  writeFileSync(
    BASELINE_PATH,
    JSON.stringify(
      { errorCount: count, updated: new Date().toISOString() },
      null,
      2
    )
  );
...
if (current.errorCount > baseline.errorCount) {
  console.error(
    `Type error count regressed: ${current.errorCount} > baseline ${baseline.errorCount}`
  );
  process.exit(1);
}
...
if (current.errorCount < baseline.errorCount) {
  console.log(
    `Great! Type errors reduced ${baseline.errorCount} -> ${current.errorCount}. Updating baseline.`
  );
  saveBaseline(current.errorCount);
  process.exit(0);
}
