...
class TreeShakingAnalyzer {
  ...
  extractImports(content, filePath) {
    ...
    const defaultImportMatches = content.matchAll(
  ...
  findUnusedExports() {
    ...
    this.stats.potentialSavingsKB = this.unusedExports.reduce(
  ...
  generateRecommendations() {
    ...
    if (this.stats.potentialSavingsKB > 20) {
      this.recommendations.push({
        type: 'general',
        description:
          'Significant tree-shaking opportunities detected - prioritize cleanup',
        priority: 'high',
        estimatedSavingsKB: this.stats.potentialSavingsKB,
      });
    }
    ...
    const packageFiles = this.unusedExports.filter(exp =>
      exp.file.startsWith('packages/')
    );
    if (packageFiles.length > 0) {
      this.recommendations.push({
        type: 'packages',
        description:
          'Clean up unused exports in shared packages to improve all apps',
        priority: 'high',
  ...
  logSummary() {
    ...
    if (this.unusedExports.length > 0) {
      ...
      for (const exp of topUnused) {
        console.log(
          `  • ${exp.name} (${exp.type}) in ${exp.file} - ${exp.reason}`
        );
      }
    }
    ...
    if (this.recommendations.length > 0) {
      ...
      for (const rec of this.recommendations) {
        console.log(
          `  ${rec.priority === 'high' ? '🔥' : '💡'} ${rec.description}`
        );
      }
    }
  }
