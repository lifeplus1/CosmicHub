...
class SmartAgentRebalancer {
  ...
  async analyzeAndRebalance() {
    ...
    const complexityAnalysis = this.calculateComplexityScores(errorDistribution);
    ...
    for (const file of agentFiles) {
      ...
      if (existsSync(filePath)) {
        try {
          ...
          const agentId = analysis.agentId || file.replace('-analysis.json', '');
  ...
  extractErrorPatterns(errorText) {
    ...
    for (const line of lines) {
      ...
      const errorMatch = line.match(/error\s+(.+?)\s+(@typescript-eslint\/[\w-]+|[\w-]+)/);
      if (errorMatch) {
  ...
  calculateComplexityScores(errorDistribution) {
    ...
    for (const [agentId, data] of Object.entries(errorDistribution)) {
      ...
      analysis[agentId] = {
        ...data,
        totalComplexity,
        complexityPerError: data.errorCount > 0 ? totalComplexity / data.errorCount : 0,
        errorBreakdown,
  ...
  createOptimizedBatches(complexityAnalysis) {
    ...
    const agentWorkloads = Object.entries(complexityAnalysis)
    ...
    for (const [agentId, analysis] of agentWorkloads) {
      ...
      if (avgComplexity <= 2) {
        ...
        optimizedBatches['medium-complexity'].agents.push({ agentId, analysis });
      } else {
  ...
  generateNewAgentConfiguration(optimizedBatches) {
    const newConfig = {
      ...
      totalAgents: Object.values(optimizedBatches).reduce((sum, batch) => sum + batch.agents.length, 0),
  ...
  generateRecommendations(newAgentConfig) {
    ...
    const batchSizes = Object.values(newAgentConfig.batches).map(b => b.agents.length);
    ...
    const totalReadyAgents = Object.values(newAgentConfig.batches)
      .reduce((sum, batch) => sum + batch.agents.filter(a => a.analysis.ready).length, 0);
  ...
  async saveRebalancedConfiguration(config) {
    const configPath = join(this.coordinationDir, 'rebalanced-agent-config.json');
...
async function main() {
  ...
  try {
    ...
    console.log('\n🔍 Complexity Analysis:');
    Object.entries(results.complexityAnalysis).forEach(([agentId, analysis]) => {
      const status = analysis.ready ? '✅' : '❌';
      console.log(`  ${status} ${agentId}: ${analysis.errorCount} errors, complexity score: ${analysis.totalComplexity.toFixed(1)}`);
    });
    
    console.log('\n⚖️  Optimized Batches:');
    Object.entries(results.optimizedBatches).forEach(([batchId, batch]) => {
      console.log(`  📦 ${batch.name}: ${batch.agents.length} agents (${batch.estimatedDuration})`);
