...
function scanFile(filePath, issues) {
  ...
  const checks = [
    ...
    {
      pattern: /<input(?![^>]*aria-label)(?![^>]*aria-labelledby)(?![^>]*id="[^"]*")(?![^>]*<label[^>]*for=)/gi,
      message: 'Input missing label or aria-label',
      severity: 'critical', 
      rule: '1.3.1'
    },
    {
      pattern: /<button(?![^>]*aria-label)(?![^>]*aria-labelledby)(?![^>]*>[^<]+)/gi,
...
function generateReport(issues) {
  ...
  // Summary
  log(`\n📈 Summary:`);
  log(`Critical Issues: ${severityCounts.critical}`, severityCounts.critical > 0 ? 'error' : 'success');
  log(`Major Issues: ${severityCounts.major}`, severityCounts.major > 0 ? 'warning' : 'success');
  log(`Minor Issues: ${severityCounts.minor}`, severityCounts.minor > 0 ? 'info' : 'success');
  log(`Total Issues: ${issues.length}`, issues.length > 0 ? 'warning' : 'success');
  ...
  for (const severity of ['critical', 'major', 'minor']) {
    ...
    if (severityIssues.length > 0) {
      ...
      severityIssues.forEach((issue, index) => {
        ...
        log(`   Code: ${issue.code.substring(0, 100)}${issue.code.length > 100 ? '...' : ''}`);
        log('');
      });
    }
  }
...
function generateMarkdownReport(issues, severityCounts) {
  const reportPath = path.join(projectRoot, 'docs/07-MONITORING/accessibility-audit-report.md');
...
`;
  ...
  log(`📄 Detailed report saved to: ${path.relative(projectRoot, reportPath)}`, 'success');
}
...
function generateNextSteps(severityCounts) {
  ...
  if (severityCounts.critical > 0) {
    steps.push('🚨 **URGENT:** Fix critical accessibility violations immediately');
...
async function runAccessibilityTests() {
  ...
  try {
    ...
    execSync('cd apps/astro && pnpm test -- src/a11y --run', { stdio: 'inherit' });
