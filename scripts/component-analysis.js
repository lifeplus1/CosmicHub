...
const ANALYSIS_OUTPUT = path.join(__dirname, '../COMPONENT-ANALYSIS-REPORT.md');
...
class ComponentAnalyzer {
  ...
  /**
   * Scan all component files across the entire project
   */
  async scanComponents() {
    ...
    for (const dir of COMPONENT_DIRECTORIES) {
      if (fs.existsSync(dir)) {
        ...
        allFiles.push(...files);
        console.log(`🔍 Found ${files.length} components in ${path.relative(process.cwd(), dir)}`);
      } else {
        console.log(`⚠️ Directory not found: ${path.relative(process.cwd(), dir)}`);
      }
    }
    
    console.log(`🔍 Analyzing ${allFiles.length} component files across the entire project...`);
      ...
      try {
        ...
        analysis.issues.forEach(issue => {
          if (issue.type === 'performance') {
            this.results.performanceIssues.push({ component: analysis.name, path: analysis.path, ...issue });
          } else if (issue.type === 'accessibility') {
            this.results.accessibilityIssues.push({ component: analysis.name, path: analysis.path, ...issue });
          } else if (issue.type === 'quality') {
            this.results.qualityIssues.push({ component: analysis.name, path: analysis.path, ...issue });
          }
        });
  ...
  generateReport() {
    ...
    const report = `# 🎯 CosmicHub Project-Wide Component Analysis Report
...
`;
  ...
  /**
   * Get components grouped by application
   */
  getComponentsByApp() {
    const byApp = {};
    this.results.needsOptimization.forEach(comp => {
    ...
    return Object.entries(byApp)
      .map(([app, components]) => `- **${app}**: ${components.length} components need optimization`)
      .join('\n');
  }
...
${items.slice(0, 5).map(item => `- **${item.component}** (\`${item.path}\`): ${item.message}`).join('\n')}
