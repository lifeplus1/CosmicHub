...
class BundleSizeMonitor {
  ...
  async analyzeApp(appName) {
    ...
    try {
      ...
      console.log(`  ✓ ${appName}: ${stats.totalSizeKB}KB (${stats.chunks.length} chunks)`);
      
    } catch (error) {
      console.error(`❌ Failed to analyze ${appName}:`, error.message);
      this.results.warnings.push(`Failed to analyze ${appName}: ${error.message}`);
    }
  }
  ...
  generateRecommendations() {
    ...
    const totalSize = this.results.apps.reduce((sum, app) => sum + app.totalSizeKB, 0);
    if (totalSize > 400) {
      this.results.recommendations.push(
  ...
  logSummary() {
    ...
    for (const app of this.results.apps) {
      console.log(`${app.name}: ${app.currentSize} (${app.change}) ${app.status}`);
