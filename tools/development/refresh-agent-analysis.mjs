...
async function refreshAgentAnalysis(agentId) {
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (existsSync(analysisFile)) {
        try {
          ...
          if (Array.isArray(analysisContent)) {
            errorCount = analysisContent.reduce(
              (sum, file) => sum + file.errorCount,
              0
            );
            warningCount = analysisContent.reduce(
      ...
      success =
        code === 0 && errorCount === 0 && warningCount <= config.maxWarnings;

      console.log(colorize(`✅ Analysis updated: ${analysisFile}`, 'green'));
      console.log(
        colorize(`⏱️  Duration: ${(duration / 1000).toFixed(2)}s`, 'blue')
      );
      console.log(
        colorize(
      ...
      console.log(
        colorize(
          `🎯 Status: ${success ? 'READY' : 'NEEDS ATTENTION'}`,
          success ? 'green' : 'red'
        )
      );
...
async function main() {
  ...
  if (!agentId) {
    ...
    console.log(
      colorize('Updates analysis files after manual ESLint fixes', 'blue')
    );
    console.log('');
    console.log(colorize('Usage:', 'bright'));
    console.log(colorize('  npm run lint:refresh-agent <agent-id>', 'cyan'));
    console.log(
      colorize('  node scripts/refresh-agent-analysis.mjs <agent-id>', 'cyan')
    );
    console.log('');
