...
const publicAllowList = [
  ...
  'VITE_STRIPE_PUBLISHABLE_KEY', // Stripe publishable keys are safe to expose
  ...
  if (fs.existsSync(serverEnvPath)) {
    ...
    if (line) {
      ...
      try {
        ...
        warnings.push(
          `Could not parse FIREBASE_CREDENTIALS JSON from .env.production.server: ${e?.message || e}`
        );
      }
    }
  }
...
for (const [k, v] of Object.entries(process.env)) {
  ...
  if (!publicAllowList.includes(k)) {
    if (
      /SECRET|PASS|PASSWORD|DATABASE|REDIS|LICENSE|KEY/i.test(k) &&
      k !== 'VITE_FIREBASE_API_KEY'
    ) {
      errors.push(
        `Potential secret exposed to client (rename without VITE_): ${k}`
      );
    } else {
      warnings.push(
        `Non-whitelisted VITE_ var: ${k} (consider allow list or rename)`
      );
    }
  }
  if (v && v.startsWith('postgres://'))
    errors.push(`Postgres connection string exposed via ${k}`);
}
...
if (
  ...
  !/^https?:\/\//.test(process.env.VITE_API_URL)
) {
...
if (
  ...
  process.env.VITE_FIREBASE_API_KEY.length < 20
) {
...
if (loadedFromFile.has('API_URL')) {
  warnings.push(
    'API_URL found in .env; move to server-only env (e.g., .env.production.server) and remove from client env'
  );
}
...
for (const k of Object.keys(process.env)) {
  if (/^(NEXT_PUBLIC_|REACT_APP_)/.test(k)) {
    warnings.push(
      `Legacy public prefix variable detected: ${k} (migrate to VITE_*)`
    );
  }
}
...
if (errors.length) {
  ...
  if (isCI && warnings.length)
    console.log('Proceeding (warnings do not fail build).');
}
