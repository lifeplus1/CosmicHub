...
const testBackendResponse = {
  ...
  houses: [
    { cusp: 0 }, { cusp: 30.5 }, { cusp: 61.25 }, { cusp: 91.75 },
    { cusp: 122.0 }, { cusp: 152.5 }, { cusp: 183.25 }, { cusp: 213.75 },
    { cusp: 244.0 }, { cusp: 274.5 }, { cusp: 305.25 }, { cusp: 335.75 }
  ],
  aspects: [
...
console.log(`  Planets: ${test1Result.planets.length} (${test1Result.planets.map(p => p.name).join(', ')})`);
console.log(`  Asteroids: ${test1Result.asteroids.length} (${test1Result.asteroids.map(a => a.name).join(', ')})`);
console.log(`  Points: ${test1Result.points.length} (${test1Result.points.map(p => p.name).join(', ')})`);
...
console.log(`  Planets: ${test2Result.planets.length} (${test2Result.planets.map(p => p.name).join(', ')})`);
console.log(`  Asteroids: ${test2Result.asteroids.length} (${test2Result.asteroids.map(a => a.name).join(', ')})`);
console.log(`  Points: ${test2Result.points.length} (${test2Result.points.map(p => p.name).join(', ')})`);
...
console.log(`  Handled gracefully: ${test3Result.planets.length === 0 ? '✅' : '❌'}`);
...
const newCalcHasUranian = test1Result.points.some(p => ['cupido', 'hades', 'zeus'].includes(p.name.toLowerCase()));
const savedChartHasData = test2Result.planets.length > 0;
const savedChartCategorized = test2Result.asteroids.length > 0 || test2Result.points.length > 0;

console.log(`✅ New calculations show asteroids: ${newCalcHasAsteroids ? '✅ PASS' : '❌ FAIL'}`);
console.log(`✅ New calculations show uranian points: ${newCalcHasUranian ? '✅ PASS' : '❌ FAIL'}`);
console.log(`✅ Saved charts processed successfully: ${savedChartHasData ? '✅ PASS' : '❌ FAIL'}`);
console.log(`✅ Saved charts properly categorized: ${savedChartCategorized ? '✅ PASS' : '❌ FAIL'}`);

const allTestsPassed = newCalcHasAsteroids && newCalcHasUranian && savedChartHasData && savedChartCategorized;
...
console.log('\n📊 SUMMARY STATISTICS:');
console.log(`  Test 1 (New Calc): ${test1Result.planets.length}P + ${test1Result.asteroids.length}A + ${test1Result.points.length}Pt`);
console.log(`  Test 2 (Saved): ${test2Result.planets.length}P + ${test2Result.asteroids.length}A + ${test2Result.points.length}Pt`);
