...
Eliminate duplicate / scattered docs in repo root by consolidating into structured `docs/` hierarchy with stubs where external links may exist.
...
`<AREA>-<ID>-<DOC_TYPE>.md` where DOC_TYPE ∈ {PLAN, IMPLEMENTATION-GUIDE, COMPLETION-SUMMARY, STATUS, REPORT}.
...
1. Should completion summaries live under architecture or a new `04-IMPLEMENTATION/`? (Currently using IMPLEMENTATION subfolder.)
