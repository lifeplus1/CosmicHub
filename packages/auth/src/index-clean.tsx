...
const loadMockUserFromStorage = (): User | null => {
  try {
    ...
    if (stored) {
      ...
      authLogger.info('🧪 Loaded mock user from storage:', userData.email);
      return userData;
    }
  } catch (error) {
    authLogger.warn('Failed to load mock user from storage:', error);
  }
  return null;
};
...
const saveMockUserToStorage = (user: User | null) => {
  try {
    if (user) {
      ...
      authLogger.info('🧪 Saved mock user to storage:', user.email);
    } else {
      sessionStorage.removeItem(MOCK_USER_KEY);
      authLogger.info('🧪 Removed mock user from storage');
    }
  } catch (error) {
    authLogger.warn('Failed to save mock user to storage:', error);
  }
};

// Initialize mock user from storage on module load
currentMockUser = loadMockUserFromStorage();
...
export const useAuth = (): AuthState => {
  const [user, setUser] = useState<User | null>(currentMockUser);
  ...
  const signOut = useCallback(async (): Promise<void> => {
    try {
      ...
      authLogger.info('🧹 Clearing mock user state');
      notifyAuthStateChange(null);
    } catch (error) {
      authLogger.error('Sign out failed:', error);
      // Even if Firebase sign out fails, clear local mock state
      authLogger.info('🧹 Force clearing mock user state after error');
      notifyAuthStateChange(null);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const mockAuthListener = (user: User | null) => {
      ...
      if (process.env['NODE_ENV'] === 'development') {
        authLogger.info('🧪 Mock auth state changed:', user ? user.email : 'null');
      }
      setUser(user);
      setLoading(false);
    };
    ...
    const initializeAuth = () => {
      ...
      if (existingMockUser) {
        authLogger.info('🧪 Found existing mock user in storage');
        currentMockUser = existingMockUser;
      ...
      if (authInstance && hasAuthAvailable) {
        try {
          unsubscribe = onAuthStateChanged(
            ...
            error => {
              if (!isComponentMounted) return;
              authLogger.error('Auth state change error:', error);
              setLoading(false);
            }
          );
        } catch (error) {
          if (!isComponentMounted) return;
          authLogger.warn('Failed to set up auth state listener:', error);
          setLoading(false);
        }
      } else {
        authLogger.info('🧪 Firebase auth not initialized, using mock auth only');
        if (isComponentMounted) {
          setLoading(false);
        }
      }
    ...
    return () => {
      ...
      if (process.env['NODE_ENV'] === 'development') {
        authLogger.info('🧹 Cleaning up auth listener');
      }
...
export async function logIn(email: string, password: string): Promise<User> {
  ...
  if (email === 'test@test.com' && password === 'test123') {
    authLogger.info('🧪 Using development mock user');
    const mockUserData = {
      uid: 'mock-user-123',
      email: 'test@test.com',
      emailVerified: true,
  ...
  if (!hasAuthAvailable) {
    authLogger.info('🧪 Firebase auth not available, using mock login');
    // Fallback to mock for development when Firebase is not available
    if (email && password) {
      authLogger.info('🧪 Using fallback mock user for development');
  ...
  try {
    ...
    authLogger.error('Firebase auth failed, trying mock login:', error);
...
export async function signUp(email: string, password: string): Promise<User> {
  try {
    ...
    authLogger.error('Firebase sign up failed, using mock:', error);
    // Fallback to mock for development
    if (email && password) {
      authLogger.info('🧪 Using mock sign up for development');
      const mockNewUser = {
...
export async function logOut(): Promise<void> {
  try {
    ...
    authLogger.error('Log out failed:', error);
    // Even if Firebase sign out fails, clear local mock state
    notifyAuthStateChange(null);
  }
}
