...
describe('useChartProcessing', () => {
    describe('Data Source Detection', () => {
        it('should detect new calculation data with __raw_backend_response', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(newCalculationData, { enableDebug: false }));
        ...
        it('should detect saved chart data with chart_data wrapper', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(savedChartData, { enableDebug: false }));
        ...
        it('should detect direct chart data without wrapper', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(directChartData, { enableDebug: false }));
            expect(result.current.source).toBe('saved_chart');
            expect(result.current.hasRawBackend).toBe(false);
        });
    });
    describe('Celestial Body Categorization', () => {
        ...
        it('should correctly categorize main planets', () => {
            const { result } = renderHook(() => useChartProcessing(sampleBackendData, { enableDebug: false }));
    ...
    describe('Edge Cases & Error Handling', () => {
        it('should handle null/undefined data gracefully', () => {
            const { result: nullResult } = renderHook(() => useChartProcessing(null, { enableDebug: false }));
            ...
            const { result: undefinedResult } = renderHook(() => useChartProcessing(undefined, { enableDebug: false }));
            expect(undefinedResult.current.source).toBe('unknown');
        });
        it('should handle invalid data types gracefully', () => {
            const { result } = renderHook(() => useChartProcessing('invalid string data', { enableDebug: false }));
        ...
        it('should handle empty objects gracefully', () => {
            const { result } = renderHook(() => useChartProcessing({}, { enableDebug: false }));
            expect(result.current.source).toBe('unknown');
            expect(result.current.planets).toHaveLength(0);
        ...
        it('should handle malformed celestial body data', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(malformedData, { enableDebug: false }));
    ...
    describe('Debug Information', () => {
        it('should provide detailed debug information', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(testData, { enableDebug: false }));
        ...
        it('should enable debug logging when requested', () => {
            ...
            const hasProcessingMessage = logCalls.some(call => typeof call === 'string' && call.includes('🔧 useChartProcessing'));
    ...
    describe('Performance & Memoization', () => {
        it('should memoize results for identical input', () => {
            const testData = { planets: { sun: { position: 30 } } };
            const { result, rerender } = renderHook(({ data }) => useChartProcessing(data, { enableDebug: false }), { initialProps: { data: testData } });
        ...
        it('should recalculate when input changes', () => {
            ...
            const { result, rerender } = renderHook(({ data }) => useChartProcessing(data, { enableDebug: false }), { initialProps: { data: testData1 } });
            const firstResult = result.current;
    ...
    describe('Critical Data Flow Fix Validation', () => {
        it('should handle the specific backend response structure from test file', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(testBackendResponse, { enableDebug: false }));
        ...
        it('should handle saved chart that gets wrapped in chart_data', () => {
            ...
            const { result } = renderHook(() => useChartProcessing(savedChartResponse, { enableDebug: false }));
            expect(result.current.source).toBe('saved_chart');
            expect(result.current.hasRawBackend).toBe(false);
...
//# sourceMappingURL=useChartProcessing.test.js.map