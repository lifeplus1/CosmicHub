 ...
 */
export function useAIInterpretationManager(options = {}) {
    const { enableCache = true, enableIndexedDB = true, cacheExpiry = 30 * 60 * 1000, // 30 minutes
    maxQueueSize = 10, fallbackToMock = true, enablePerformanceMonitoring = true, } = options;
    ...
    const cacheKey = useCallback((request) => {
        return `interpretation:${request.chartId}:${request.userId}:${request.interpretationType}`;
    }, []);
    const getCachedInterpretation = useCallback(async (request) => {
        if (!enableCache)
            return null;
    ...
    const setCachedInterpretation = useCallback(async (request, interpretation) => {
        if (!enableCache)
            return;
        const key = cacheKey(request);
        // Set memory cache
    ...
    // API call utilities
    const callInterpretationAPI = useCallback(async (request) => {
    ...
    const processQueue = useCallback(async () => {
        if (state.processingQueue || state.queue.length === 0)
            return;
    ...
    // Public API methods
    const requestInterpretation = useCallback((request) => {
    ...
    const getInterpretationById = useCallback((id) => {
        return state.interpretations.find(interp => interp.id === id) || null;
    }, [state.interpretations]);
    const getInterpretationsByChart = useCallback((chartId) => {
        return state.interpretations.filter(interp => interp.chartId === chartId);
    }, [state.interpretations]);
    const clearInterpretations = useCallback(() => {
        setState(prev => ({
            ...prev,
            interpretations: [],
            error: null,
...
function generateMockInterpretationContent(type) {
    const mockContent = {
    ...
    return mockContent[type] || mockContent.general;
}
//# sourceMappingURL=useAIInterpretationManager.js.map