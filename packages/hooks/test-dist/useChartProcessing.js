...
const calculateHousePosition = (planetDegrees, houseCusps) => {
    ...
    for (let i = 0; i < houseCusps.length; i++) {
        ...
        if (currentCuspValue === null || currentCuspValue === undefined ||
            nextCuspValue === null || nextCuspValue === undefined)
            continue;
 ...
 */
export function useChartProcessing(chartData, options = {}) {
    ...
    const debugRef = useRef({ lastProcessedId: null });
    return useMemo(() => {
        ...
        const asteroidCount = hasAsteroids ? Object.keys(rawBackendData.asteroids).length : 0;
        const pointCount = [
            hasPoints ? Object.keys(rawBackendData.points).length : 0,
            hasUranian ? Object.keys(rawBackendData.uranian).length : 0,
            hasHypothetical ? Object.keys(rawBackendData.hypothetical_points).length : 0,
...
function processChartDataInternal(rawData, enableDebug, useModernRulers = true) {
    ...
    ['planets', 'asteroids', 'points', 'uranian', 'hypothetical_points'].forEach(fieldName => {
        ...
        if (fieldData === null || fieldData === undefined || typeof fieldData !== 'object')
            return;
...
//# sourceMappingURL=useChartProcessing.js.map