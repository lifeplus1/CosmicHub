...
describe('Chart Processing Data Flow Validation', () => {
  describe('Data Structure Validation', () => {
    ...
    it('should have expected structure for API transformed data', () => {
      ...
      expect(mockApiTransformedData.__raw_backend_response).toBe(
        mockRawBackendData
      );
    });
  ...
  describe('Critical Data Flow Issue Validation', () => {
    it('should demonstrate the core problem we are solving', () => {
      ...
      console.log(
        ...
        '__raw_backend_response' in mockApiTransformedData
      );
      console.log(
        '   - Raw backend planets:',
        Object.keys(mockRawBackendData.planets)
      );
      console.log(
        '   - Raw backend asteroids:',
        Object.keys(mockRawBackendData.asteroids)
      );
      console.log(
        '   - Raw backend points:',
        Object.keys(mockRawBackendData.points)
      );
      console.log(
        '   - Raw backend uranian:',
        Object.keys(mockRawBackendData.uranian)
      );

      console.log('2. Saved chart data (from /api/charts/ endpoint):');
      console.log(
        '   - Has __raw_backend_response:',
        '__raw_backend_response' in mockSavedChartData
      );
      ...
      console.log(
        '🎯 THE FIX: useChartProcessing hook should handle BOTH sources!'
      );
      ...
      expect(Object.keys(mockRawBackendData.hypothetical_points)).toHaveLength(
        1
      );
    });
  });
