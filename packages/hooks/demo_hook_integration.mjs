...
console.log('1. New calculation data:', {
  ...
  apiTransformedFields: Object.keys(newCalculationData).filter(
    k => k !== '__raw_backend_response'
  ),
});
...
console.log(
  '✅ For NEW calculations: Should use __raw_backend_response for proper categorization'
);
console.log(
  '✅ For SAVED charts: Should process directly with fallback categorization logic'
);
console.log(
  '✅ Should categorize: 2 planets, 3 asteroids, 6+ points (3 special + 2 uranian + 1 hypothetical)'
);
...
function simulateHookLogic(chartData, label) {
  ...
  if (rawData.hypothetical_points)
    Object.assign(points, rawData.hypothetical_points);
  ...
  console.log(
    '  Houses:',
    Array.isArray(rawData.houses) ? rawData.houses.length : 'N/A'
  );
...
console.log(
  '✅ New calculation processed:',
  newCalcResult.hasRawBackend ? '✅ PASS' : '❌ FAIL'
);
console.log(
  '✅ Saved chart processed:',
  !savedResult.hasRawBackend ? '✅ PASS' : '❌ FAIL'
);
console.log(
  '✅ Proper categorization:',
  newCalcResult.planetsCount === 2 &&
    newCalcResult.asteroidsCount === 3 &&
    newCalcResult.pointsCount >= 6
    ? '✅ PASS'
    : '❌ FAIL'
);
...
console.log(
  'Both new calculations and saved charts will be processed correctly by useChartProcessing hook.'
);
