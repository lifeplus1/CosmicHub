...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  'id.invalid_string':
    'Experiment ID must contain only letters, numbers, underscores, and hyphens',
  ...
  'metrics.guardrails.too_small':
    'At least one guardrail metric is recommended for safety',

  // Date validation
  'start_date.invalid_string':
    'Start date must be in ISO 8601 format (e.g., 2025-08-25T10:00:00Z)',
  'end_date.invalid_string':
    'End date must be in ISO 8601 format (e.g., 2025-08-25T10:00:00Z)',
  ...
  // Variants validation
  'variants.too_small':
    'Experiment must have at least 2 variants (control and treatment)',
  'variants.too_big':
    'Too many variants - consider splitting into separate experiments (maximum 10)',
  ...
  'status.invalid_enum_value':
    'Status must be one of: planned, running, completed, aborted, analyzing',
};
...
export function validateExperiment(
  data: unknown
): ValidationResult<ExperimentRegistry> {
  try {
    ...
    if (result.success) {
      ...
      const controlVariants = result.data.variants.filter(
        v =>
      ...
      if (controlVariants.length === 0) {
        warnings.push(
          'Consider having a clear control/baseline variant for comparison'
        );
      }
      ...
      if (durationDays < 7) {
        warnings.push(
          'Experiments shorter than 1 week may not capture weekly patterns'
        );
      } else if (durationDays > 90) {
        warnings.push(
          'Long experiments may be affected by external factors - consider shorter duration'
        );
      }
 ...
 */
export function validateExperimentBatch(
  data: unknown
): ValidationResult<ExperimentBatch> {
  try {
    ...
    if (result.success) {
      ...
      result.data.experiments.forEach((exp, index) => {
        ...
        if (validation.success && validation.warnings) {
          warnings.push(
            ...validation.warnings.map(w => `Experiment ${index + 1}: ${w}`)
          );
        }
      });
      ...
      error: {
        message:
          error instanceof Error
            ? error.message
            : 'Unknown batch validation error',
        issues: [],
        code: 'VALIDATION_ERROR',
      },
    };
 ...
 */
export function formatValidationErrors<T>(
...
export function canStartExperiment(
  ...
  const hasInvalidVariants = experiment.variants.some(
    variant => !variant.config || Object.keys(variant.config).length === 0
  );
...
export class ExperimentMigration {
  ...
  static migrateV1toV2(
    experimentV1: Record<string, unknown>
  ): ExperimentRegistry {
    const migrated = {
      ...experimentV1,
  ...
  static migrate(experiment: Record<string, unknown>): ExperimentRegistry {
    ...
    switch (version) {
      ...
      default: {
        ...
        if (!validation.success) {
          throw new Error(
            `Invalid experiment data: ${validation.error.message}`
          );
        }
        return validation.data;
      }
...
export const ExperimentValidation = {
  ...
  isValidStatus: (status: string): status is keyof typeof ExperimentStatus => {
    return Object.values(ExperimentStatus).includes(
      status as (typeof ExperimentStatus)[keyof typeof ExperimentStatus]
    );
  },
  ...
  isValidTrafficAllocation: (
    variants: { traffic_percentage: number }[]
  ): boolean => {
