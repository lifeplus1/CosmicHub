...
Note: Respect privacy - provide general pattern insights without identifying specific individuals.`;
  ...
  static async generateMockResponse(requestType: string, request: any): Promise<string> {
    ...
    const mockResponses = {
      'transit-analysis': `🌟 TRANSIT ANALYSIS - ${request.timeframe?.toUpperCase() || 'CUSTOM'} FORECAST
...
💰 Financial planning: Excellent timing for investments in ${request.focusAreas?.includes('finances') ? '2 weeks' : 'near future'}

Action Plan:
1. IMMEDIATE (Next 2 weeks): Focus on ${request.focusAreas?.[0] || 'career'} opportunities
...
Cosmic timing is strongly supportive for your goals in ${request.focusAreas?.join(', ') || 'all areas'}.`,
...
🌟 **Direct Answer**: ${request.question?.includes('career') ? 'Your career path is illuminated by strong Jupiter influences, suggesting expansion and growth opportunities ahead.' : 'The planetary positions in your chart provide clear guidance for this situation.'}

🪐 **Astrological Context**: Your chart shows ${request.context?.userProfile ? 'strong emphasis on personal development and authentic self-expression' : 'significant potential for growth and positive change'}. The current planetary transits are particularly supportive of your intentions.
...
Is there a particular aspect of this guidance you'd like me to elaborate on?`,
      ...
      'multi-system-synthesis': `🌍 MULTI-SYSTEM SYNTHESIS - ${request.synthesisType?.toUpperCase() || 'COMPREHENSIVE'} ANALYSIS

Integration of ${request.systems?.join(', ').toUpperCase() || 'MULTIPLE'} Astrological Traditions:

🔯 **Western Astrology Insights**: 
Strong emphasis on ${request.synthesisType || 'personal development'} with key placements supporting growth and expansion.

🕉️ **Vedic Astrology Perspective**: 
Your nakshatra indicates a soul purpose aligned with ${request.synthesisType === 'career' ? 'service and expertise sharing' : 'spiritual development and wisdom teaching'}.

🐉 **Chinese Astrology Wisdom**: 
Your animal sign and element combination suggests ${request.synthesisType === 'relationships' ? 'harmony through balance and patience' : 'success through persistent effort and strategic timing'}.

🏛️ **Unified Interpretation**: 
All systems converge on a theme of ${request.depth === 'advanced' ? 'transformational leadership and spiritual service' : 'personal growth and authentic expression'}. The integration reveals:
...
🌟 **Cross-Cultural Wisdom**: Each tradition emphasizes your role as a connector and wisdom keeper, suggesting your greatest fulfillment comes through helping others understand deeper truths.`,
      ...
      'personal-growth': `🌱 PERSONAL GROWTH COACHING - ${request.growthArea?.replace('_', ' ').toUpperCase() || 'COMPREHENSIVE'} DEVELOPMENT

Astrological Growth Analysis:

🎯 **Your Growth Profile**: 
Your birth chart reveals natural gifts in ${request.growthArea || 'personal development'} with strong planetary support for transformation and expansion.

🔍 **Challenge Perspective**: 
Current obstacles are actually cosmic invitations to develop:
${request.currentChallenges?.map(c => `• ${c} - Opportunity for mastery and wisdom`).join('\n') || '• Patience and self-compassion\n• Trust in divine timing\n• Integration of shadow aspects'}

💪 **Strength Activation Plan**:
1. **Natural Talents**: Your chart shows gifts in intuition, communication, and healing
2. **Cosmic Support**: Current planetary transits favor ${request.timeframe || 'gradual'} transformation
...
Your ${request.timeframe || 'immediate'} growth potential is exceptionally strong right now!`,

      'pattern-recognition': `📊 PATTERN RECOGNITION ANALYSIS - ${request.patternTypes?.join(', ').toUpperCase() || 'COMPREHENSIVE'} PATTERNS

Collection Analysis of ${request.chartCollection?.length || 'Multiple'} Charts:
...
💫 **Collective Insights**:
This chart collection represents a group with shared spiritual mission around ${request.analysisDepth === 'comprehensive' ? 'consciousness evolution and planetary healing' : 'personal and collective transformation'}. Strong indicators suggest:
