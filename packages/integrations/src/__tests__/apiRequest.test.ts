...
describe('integrations apiRequest', () => {
  ...
  it('returns success on ok response', async () => {
    global.fetch = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({ value: 5 }) });
  ...
  it('returns failure on non-ok response', async () => {
    global.fetch = vi.fn().mockResolvedValue({ ok: false, status: 500, json: async () => ({ message: 'bad' }) });
