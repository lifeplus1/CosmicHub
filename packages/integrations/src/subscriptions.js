...
class SubscriptionManager extends SimpleEventEmitter {
    ...
    checkFeatureAccess(feature, app) {
        ...
        if (app && !plan.apps.includes(app)) {
            const requiredPlan = this.plans.find(p => p.apps.includes(app) && p.features.includes(feature));
            return {
                canAccess: false,
        ...
        if (!hasFeature) {
            const requiredPlan = this.plans.find(p => p.features.includes(feature) && (!app || p.apps.includes(app)));
            return {
                canAccess: false,
    ...
    expiresWithin(days) {
        ...
        const daysUntilExpiry = Math.ceil((this.currentSubscription.currentPeriodEnd.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
...
export const useSubscription = () => {
    const [subscription, setSubscription] = React.useState(subscriptionManager.getCurrentSubscription());
    const [plan, setPlan] = React.useState(subscriptionManager.getCurrentPlan());
    React.useEffect(() => {
