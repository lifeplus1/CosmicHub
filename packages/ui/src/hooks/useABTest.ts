  ...
  return (eventName: string, properties: ABTestEventProperties = {}) => {
    ...
    try {
      ...
      events = Array.isArray(parsed)
        ? (parsed.filter(v => typeof v === 'object' && v !== null) as Array<
    ...
    try {
      ...
      const mixpanel: Mixpanel | undefined =
        typeof window !== 'undefined'
      ...
      if (typeof window !== 'undefined' && import.meta.env.PROD) {
        Promise.all([
          ...
          .then(([analyticsMod, firebaseConfig]) => {
            ...
            const container =
              modAny['default'] && typeof modAny['default'] === 'object'
                ? (modAny['default'] as Record<string, unknown>)
                : modAny;
            const getAnalytics = container['getAnalytics'] as
              | ((app: unknown) => unknown)
              | undefined;
            const logEvent = container['logEvent'] as
              | ((
                  inst: unknown,
                  name: string,
                  params?: Record<string, unknown>
