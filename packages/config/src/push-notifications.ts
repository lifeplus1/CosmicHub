...
export class PushNotificationManager {
  ...
  async subscribeUser(
    ...
    try {
      ...
      if (permission !== 'granted') {
        (
          globalThis as unknown as {
            devConsole?: { log?: (...args: unknown[]) => void };
          }
        ).devConsole?.log?.('❌ Notification permission denied');
        return null;
      }
      ...
      const subKeys = subscription.toJSON().keys as {
        auth: string;
        p256dh: string;
      };
      ...
      ).devConsole?.log?.('✅ User subscribed to push notifications');
      return userSubscription;
    } catch (error) {
      (
        globalThis as unknown as {
          devConsole?: { error?: (...args: unknown[]) => void };
        }
      ).devConsole?.error?.('❌ Failed to subscribe user:', error);
      return null;
    }
  }
  ...
  async unsubscribeUser(userId: string): Promise<boolean> {
    try {
      ...
      ).devConsole?.log?.('✅ User unsubscribed from push notifications');
      return true;
    } catch (error) {
      (
        globalThis as unknown as {
      ...
      ).devConsole?.error?.('❌ Failed to unsubscribe user:', error);
      return false;
    }
  }
  ...
  async queueNotification(notification: NotificationPayload): Promise<void> {
    ...
    if (this.isOnline) {
      ...
      ).devConsole?.log?.('📤 Notification queued for when online');
    }
  }
  ...
  private async processNotificationQueue(): Promise<void> {
    ...
    for (const notification of notifications) {
      try {
        ...
        ).devConsole?.error?.('❌ Failed to process notification:', error);
  ...
  async suggestNotificationSettings(
    userId: string
    ...
    const hoursSinceLastUse =
      (Date.now() - subscription.lastUsed) / (1000 * 60 * 60);
  ...
  getNotificationStats(): NotificationStats {
    const activeSubscriptions = Array.from(this.subscriptions.values()).filter(
      sub => Date.now() - sub.lastUsed < 30 * 24 * 60 * 60 * 1000
  ...
  private setupConnectionListener(): void {
    window.addEventListener('online', () => {
      ...
      ).devConsole?.log?.('🌐 Back online - processing notification queue');
    ...
    window.addEventListener('offline', () => {
      ...
      ).devConsole?.log?.('📴 Gone offline - notifications will queue');
    });
  }
  ...
  private saveSubscriptions(): void {
    try {
      ...
      localStorage.setItem(
        'cosmichub-push-subscriptions',
        JSON.stringify(data)
      );
    } catch (error) {
      (
        globalThis as unknown as {
          devConsole?: { warn?: (...args: unknown[]) => void };
        }
      ).devConsole?.warn?.('Failed to save subscriptions:', error);
    }
  }
  ...
  private loadStoredSubscriptions(): void {
    try {
      ...
      ).devConsole?.warn?.('Failed to load stored subscriptions:', error);
    }
  }
}
...
export class AstrologyNotificationScheduler {
  ...
  scheduleDailyHoroscope(userId: string, sign: string, time: string): void {
    ...
    (
      ...
      `📅 Scheduled daily horoscope for user ${userId}, sign ${sign} at ${time}`,
      { pushManager: Boolean(this._pushManager) }
    );
  }
  ...
  scheduleTransitAlerts(userId: string): void {
    ...
    ).devConsole?.log?.(`🔮 Scheduled transit alerts for user ${userId}`);
  }
  ...
  scheduleMoonPhases(userId: string): void {
    ...
    (
      ...
      `🌙 Scheduled moon phase notifications for user ${userId}`
    );
  }
}
