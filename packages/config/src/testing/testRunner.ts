...
class TestSuiteRunner {
  ...
  constructor(
    ...
    bus: TestEventBus = createDefaultEventBus()
  ) {
    this.config = {
      coverage: {
        threshold: 80,
  ...
  async runSuite(
    ...
    const errors: string[] = [];
    let renderTime = 0;
    const mountTime = 0; // Kept const as it's never reassigned
    ...
    try {
      ...
      this.bus.emit({
        type: 'error',
        message: `Test suite failed: ${suiteName}`,
        suite: suiteName,
        error: msg,
      } as ErrorEvent);
    }
  ...
  async runAllSuites(
    suites: Record<string, () => Promise<void>>
  ...
  private generateSummary(): TestRunSummary {
    ...
    const avgCoverage = this.results.reduce(
      (acc, result) => {
    ...
    const totalMemoryUsage = this.results.reduce(
      (acc, r) => acc + r.performance.memoryUsage,
      0
    );

    // Calculate accessibility metrics
    const totalViolations = this.results.reduce(
      (acc, r) => acc + r.accessibility.violations.length,
      0
    );
    const criticalViolations = this.results.reduce(
      (acc, r) =>
        acc +
        r.accessibility.violations.filter(v => v.impact === 'critical').length,
      0
    );
    const totalWarnings = this.results.reduce(
      (acc, r) => acc + r.accessibility.warnings.length,
      0
    );
  ...
  private calculateQualityMetrics(metrics: {
    ...
    const coverageScore = Math.min(
      (metrics.coverage / this.config.coverage.threshold) * 100,
      100
    );
    const performanceScore =
      metrics.performance <= this.config.performance.maxRenderTime
        ? 100
        : Math.max(
            0,
            100 -
              (metrics.performance - this.config.performance.maxRenderTime) * 5
          );
    const accessibilityScore =
      metrics.accessibility === 0
        ? 100
        : Math.max(0, 100 - metrics.accessibility * 10);
  ...
  private getMemoryUsage(): number {
    // Safely access memory usage across environments
    const processObject =
    ...
    return processObject?.memoryUsage?.()?.heapUsed ?? 0;
  }

  private getProcessInfo(): { version: string; platform: string } {
    // Safely access process info across environments
    const processObject =
      typeof globalThis !== 'undefined' && 'process' in globalThis
        ? (
            globalThis as unknown as {
    ...
    return {
      version: processObject?.version ?? 'unknown',
      platform: processObject?.platform ?? 'unknown',
    };
  }
  ...
  private generateMockAccessibility(): {
    ...
    warnings: Array<{ id: string; description: string; nodes: number }>;
  } {
  ...
  private emitResultWarnings(result: TestResult): void {
    if (result.performance.renderTime > this.config.performance.maxRenderTime) {
      this.bus.emit({
        type: 'warning',
        message: `Slow render time: ${result.performance.renderTime.toFixed(2)}ms`,
        suite: result.suite,
        code: 'PERF_SLOW_RENDER',
      } as WarningEvent);
    }
    if (result.accessibility.violations.length > 0) {
      this.bus.emit({
        type: 'warning',
        message: `${result.accessibility.violations.length} accessibility violations`,
        suite: result.suite,
        code: 'A11Y_VIOLATIONS',
      } as WarningEvent);
    }
  }
  ...
  private async ensureOutputDir(subdir?: string): Promise<string> {
    const baseDir = this.config.reports.outputDir ?? 'test-results';
    // Normalize subdir (treat empty/whitespace as undefined)
    const clean =
      typeof subdir === 'string' && subdir.trim().length > 0
        ? subdir.trim()
        : undefined;
    const dirPath = clean !== undefined ? `${baseDir}/${clean}` : baseDir;
    try {
  ...
  private async generateJsonReport(summary: TestRunSummary): Promise<void> {
    ...
    try {
      ...
      this.bus.emit({
        ...
        format: 'json',
        location: path,
      } as ReportGeneratedEvent);
    } catch {
      this.bus.emit({
        type: 'error',
        message: 'Failed to write JSON report',
        error: path,
      } as ErrorEvent);
    }
  }
  ...
  private async generateHtmlReport(summary: TestRunSummary): Promise<void> {
    ...
    try {
      ...
      this.bus.emit({
        type: 'error',
        message: 'Failed to write HTML report',
        error: path,
      } as ErrorEvent);
    }
  }
  ...
  private buildHtmlReport(summary: TestRunSummary): string {
    const rows = this.results
      .map(
        r =>
          `<tr><td>${r.suite}</td><td>${r.status}</td><td>${r.duration.toFixed(2)}ms</td><td>${r.coverage.statements.toFixed(1)}%</td><td>${r.performance.renderTime.toFixed(2)}ms</td><td>${r.accessibility.violations.length}</td></tr>`
      )
      .join('\n');
    const recommendations = summary.quality.recommendations
      .map(r => `<li>${r}</li>`)
      .join('\n');
...
export const createDevelopmentRunner = (): TestSuiteRunner =>
  new TestSuiteRunner({
