...
export function withPerformanceTracking<P extends Record<string, unknown>>(
  ...
  const Tracked: FC<P> = props => {
    ...
    React.useEffect(() => {
      ...
      return () => {
        logger.debug('component.unmount', { component: name });
      };
    }, []);
...
export function createCompoundComponent<
  T extends ComposableComponentProps & Record<string, unknown>,
>(Base: ComponentType<T>, name: string): FC<T> & CompoundComponentAPI {
  const Header: FC<ComposableComponentProps> = ({
    children,
    className = '',
    ...rest
  }) => (
    <div className={`compound-header ${className}`} {...rest}>
      {children}
    </div>
  );
  const Body: FC<ComposableComponentProps> = ({
    children,
    className = '',
    ...rest
  }) => (
    <div className={`compound-body ${className}`} {...rest}>
      {children}
    </div>
  );
  const Footer: FC<ComposableComponentProps> = ({
    children,
    className = '',
    ...rest
  }) => (
    <div className={`compound-footer ${className}`} {...rest}>
      {children}
    </div>
  );
  const Actions: FC<ComposableComponentProps> = ({
    children,
    className = '',
    ...rest
  }) => (
    <div className={`compound-actions ${className}`} {...rest}>
      {children}
    </div>
  );
...
> = PolymorphicProps<T> &
  P &
  Omit<React.ComponentPropsWithoutRef<T>, keyof PolymorphicProps<T> | keyof P>;
export interface PolymorphicForwardComponent<TDefault extends ElementType> {
  <TAs extends ElementType = TDefault>(
    props: PolymorphicComponentProps<TAs> & {
...
export function createPolymorphicComponent<
  TDefault extends ElementType = 'div',
>(
  defaultTag: TDefault,
  displayName?: string
): PolymorphicForwardComponent<TDefault> {
  type AnyElement = ElementType;
  // Define the inner component with proper generic typing
  const Poly = (({
    as,
    ...rest
  }: { as?: AnyElement } & Record<string, unknown>) => {
...
export class ComponentPerformanceAnalyzer {
  ...
  static getInstance(): ComponentPerformanceAnalyzer {
    return (
      this.instance ?? (this.instance = new ComponentPerformanceAnalyzer())
    );
  }
  ...
  generateRecommendations(component: string): string[] {
    ...
    const rec: string[] = [];
    if (
      a['ComponentRender']?.average !== undefined &&
      a['ComponentRender'].average > 16
    ) {
      rec.push(`Optimize ${component} render time`);
    }
    if (
      a['ComponentMount']?.average !== undefined &&
      a['ComponentMount'].average > 100
    ) {
...
export function useComponentAnalysis(name: string): {
  ...
  const recordMetric = useCallback(
    (metric: string, value: number) =>
      analyzer.recordComponentMetric(name, metric, value),
    [analyzer, name]
  );
  return {
    recordMetric,
...
>(): ComponentFactory<TConfig> {
  ...
  return {
    ...
    register(name, component): void {
      reg.set(name, component);
      logger.debug('component.register', { name, size: reg.size });
    },
...
export function withMemoization<P extends Record<string, unknown>>(
  ...
  if (strategy.custom)
    return memo(
      Component,
      strategy.custom as (a: P, b: P) => boolean
