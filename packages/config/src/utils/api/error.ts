...
export function isStandardFailure<T>(r: StandardApiResponse<T>): r is StandardFailure {
  return r.success === false;
}

export function buildFailure(message: string, code?: string, details?: unknown): StandardFailure {
...
export function parseErrorLike(input: unknown, fallbackCode = 'UNKNOWN'): StandardApiError {
  if (typeof input === 'object' && input !== null) {
    const rec = input as Record<string, unknown>;
    const code = typeof rec.code === 'string' ? rec.code : fallbackCode;
    const message = typeof rec.message === 'string' ? rec.message : 'API request failed';
