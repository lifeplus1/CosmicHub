...
export const toFailure = (
  error: unknown,
  opts: FailureMapOptions
): ApiFailure => {
  if (typeof error === 'object' && error !== null) {
    const maybeResp = (error as { response?: { status?: unknown } }).response;
    const status =
      typeof maybeResp?.status === 'number' ? maybeResp.status : undefined;
    if (status === 401)
      return fail(opts.auth ?? 'Authentication required', ErrorCode.AUTH);
    if (status === 404)
      return fail(opts.notFound ?? 'Resource not found', ErrorCode.NOT_FOUND);
    if (status === 400)
      return fail(opts.validation ?? 'Validation error', ErrorCode.VALIDATION);
  }
  logger.once(
    'warn',
    'apiresult.fallback',
    'ApiResult toFailure fallback path used',
...
export const isFailure = <T>(r: ApiResult<T>): r is ApiFailure =>
  r.success === false;
...
export const unwrapOr = <T>(r: ApiResult<T>, fallback: T): T =>
...
export const mapSuccess = <A, B>(
  r: ApiResult<A>,
  fn: (data: A) => B
): ApiResult<B> => (r.success ? ok(fn(r.data), r.message) : r) as ApiResult<B>;

// mapFailure: transform failure error/code
export const mapFailure = <T>(
  r: ApiResult<T>,
  fn: (f: ApiFailure) => ApiFailure
): ApiResult<T> => (r.success ? r : fn(r));
