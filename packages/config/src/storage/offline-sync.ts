...
export const saveChartOfflineWithSync = async (chartData: unknown, birthData?: unknown, userId?: string): Promise<void> => {
  const manager = getOfflineSyncManager();
  await manager.saveChartWithSync(chartData, birthData ?? {}, userId ?? 'anonymous');
};

export const deleteChartOfflineWithSync = async (chartId: string): Promise<void> => {
  const manager = getOfflineSyncManager();
  await manager.deleteChartWithSync(chartId);
};
...
export const getUserChartsOffline = async (userId?: string): Promise<unknown[]> => {
  const manager = getOfflineSyncManager();
  return await manager.getUserCharts(userId ?? 'anonymous', false);
};
