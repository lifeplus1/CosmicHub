...
export function useAnalytics(): UseAnalyticsReturn {
  ...
  const logEventCb = useCallback<UseAnalyticsReturn['logEvent']>(
  ...
  const setUserIdCb = useCallback<UseAnalyticsReturn['setUserId']>(
    userId => {
      if (!analytics) return;
      void import('firebase/analytics')
  ...
  const setUserPropertiesCb = useCallback<
    UseAnalyticsReturn['setUserProperties']
  >(
    props => {
      if (!analytics) return;
      void import('firebase/analytics')
  ...
  const setCurrentScreenCb = useCallback<
    UseAnalyticsReturn['setCurrentScreen']
  >(
    screen => {
      if (!analytics) return;
      void import('firebase/analytics')
