...
const warnDev = (...args: unknown[]): void => {
  if (import.meta.env?.DEV) {
    console.warn(...args);
  }
};
const errorDev = (...args: unknown[]): void => {
  console.error(...args);
};
...
function isPerformanceMonitor(value: unknown): value is PerformanceMonitorLike {
  return (
    typeof value === 'object' &&
    value !== null &&
    typeof (value as Record<string, unknown>).getPerformanceReport ===
      'function' &&
    typeof (value as Record<string, unknown>).enableRealTimeUpdates ===
      'function'
  );
}
...
export function usePerformance(): {
  ...
  const end = useCallback((): PerformanceMetrics | null => {
    ...
    const newMetrics: PerformanceMetrics = {
      ...
      endTime,
      memory: (performance as unknown as { memory?: PerformanceMemoryLike })
...
export function useOperationTracking(): {
  ...
  const operations = useMemo(
    () => Array.from(operationsMap.values()),
    [operationsMap]
  );

  const startOperation = useCallback(
    (operationName: string, metadata?: Record<string, unknown>): string => {
  ...
  const endOperation = useCallback(
    (operationId: string, errorMessage?: string): void => {
  ...
  const getOperationsByStatus = useCallback(
    (status: OperationMetrics['status']): OperationMetrics[] =>
  ...
  const getOperationStats = useCallback(() => {
    ...
    const avgDuration =
      completed.length > 0
        ? completed.reduce((s, o) => s + (o.duration ?? 0), 0) /
          completed.length
        : 0;
    return {
    ...
    const collectMetrics = (): void => {
      try {
        ...
        const firstContentfulPaint =
          fcpEntry !== undefined && fcpEntry !== null ? fcpEntry.startTime : 0;
  ...
  useEffect(() => {
    ...
    const setup = async (): Promise<void> => {
      try {
        ...
        cleanup = pmCandidate.enableRealTimeUpdates(r => {
          if (mounted && isPerformanceReport(r)) setReport(r);
        });
      } catch (err) {
