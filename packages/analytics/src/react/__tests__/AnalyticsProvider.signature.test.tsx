...
describe('AnalyticsProvider config signature', () => {
  it('does not re-init when irrelevant fields change', () => {
    const { rerender } = render(<AnalyticsProvider config={baseConfig}>{null}</AnalyticsProvider>);
    expect(getAnalytics).toHaveBeenCalledTimes(1);
    expect(initializeAnalytics).toHaveBeenCalledTimes(0);

    // Change a field not included in signature: add piiKeys
    const updated: AnalyticsConfig = { ...baseConfig, advanced: { ...baseConfig.advanced, piiKeys: ['email'] } };
  ...
  it('re-inits when a signature field changes', () => {
    ...
    const withNewGA: AnalyticsConfig = { ...baseConfig, googleAnalytics: { ...baseConfig.googleAnalytics!, measurementId: 'GA-2', enabled: true } };
  ...
  it('re-inits when advanced sessionTimeoutMs changes', () => {
    const { rerender } = render(<AnalyticsProvider config={baseConfig}>{null}</AnalyticsProvider>);
    const changed: AnalyticsConfig = { ...baseConfig, advanced: { ...baseConfig.advanced, sessionTimeoutMs: 42 } };
