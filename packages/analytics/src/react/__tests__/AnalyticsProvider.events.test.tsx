...
const baseConfig: AnalyticsConfig = {
  ...
  posthog: { apiKey: 'PH-1', enabled: true, sessionRecording: false, heatmaps: false },
  customAnalytics: { endpoint: 'https://api.example.test/ingest', enabled: true },
  privacy: { respectDoNotTrack: true, anonymizeIP: true, cookieConsent: true, dataRetentionDays: 30 },
  advanced: { sessionTimeoutMs: 1800000, autoFlushIntervalMs: 10000, autoTrackErrors: true }
};
...
const FireTrack: React.FC<{ ev: Partial<AnalyticsEvent> }>= ({ ev }) => {
  ...
  return lastEvent ? <div data-testid="last-event" data-event={lastEvent.event}></div> : null;
};
...
describe('AnalyticsProvider events', () => {
  it('updates lastEvent and notifies subscribers via augmented onDispatch', async () => {
    ...
    const { findByTestId } = render(<AnalyticsProvider config={baseConfig}><Wrapper /></AnalyticsProvider>);
  ...
  it('re-initializes when onDispatch function identity changes', () => {
    const { rerender } = render(<AnalyticsProvider config={{ ...baseConfig, advanced: { ...baseConfig.advanced, onDispatch: () => {} } }}>{null}</AnalyticsProvider>);
    expect(initializeAnalytics).toHaveBeenCalledTimes(1);
    rerender(<AnalyticsProvider config={{ ...baseConfig, advanced: { ...baseConfig.advanced, onDispatch: () => {} } }}>{null}</AnalyticsProvider>);
  ...
  it('avoids re-init when onDispatch is stable (memoized reference)', () => {
    const stable = () => {};
    const firstConfig = { ...baseConfig, advanced: { ...baseConfig.advanced, onDispatch: stable } } as AnalyticsConfig;
    const secondConfig = { ...baseConfig, advanced: { ...baseConfig.advanced, onDispatch: stable } } as AnalyticsConfig;
    const { rerender } = render(<AnalyticsProvider config={firstConfig}>{null}</AnalyticsProvider>);
    expect(initializeAnalytics).toHaveBeenCalledTimes(1);
    rerender(<AnalyticsProvider config={secondConfig}>{null}</AnalyticsProvider>);
    expect(initializeAnalytics).toHaveBeenCalledTimes(1);
  });
});
