...
const baseConfig = (over: Partial<AnalyticsConfig> = {}): AnalyticsConfig => ({
  ...
  posthog: { apiKey: 'ph', enabled: true, sessionRecording: false, heatmaps: false },
  segment: { writeKey: 'seg', enabled: true },
  rudderstack: { writeKey: 'rudder', dataPlaneUrl: 'https://rudder', enabled: true },
  customAnalytics: { endpoint: '/ingest', enabled: true },
  privacy: { respectDoNotTrack: false, anonymizeIP: true, cookieConsent: true, dataRetentionDays: 30 },
  advanced: { sessionTimeoutMs: 1000 * 60 * 30, autoFlushIntervalMs: 5000, autoTrackErrors: true, ...over.advanced },
  ...over
});

// eslint-disable-next-line no-unused-vars
const fireDispatch = (onDispatch: ((event: AnalyticsEvent) => void) | undefined) => {
  onDispatch?.({ event: 'test', session_id: 's', timestamp: Date.now(), properties: { a: 1 }, platform: 'web' });
};
...
describe('AnalyticsProvider config signature + subscription', () => {
  it('re-initializes when advanced.onDispatch identity changes', () => {
    ...
    const { rerender } = render(<AnalyticsProvider config={cfg1}><div>child</div></AnalyticsProvider>);
    rerender(<AnalyticsProvider config={cfg2}><div>child</div></AnalyticsProvider>);
