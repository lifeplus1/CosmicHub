  ...
  if (!(globalThis as any).document) {
    (globalThis as any).document = {
      ...
      createElement: (tag: string) => ({
        ...
        onload: null as any,
        onerror: null as any,
        addEventListener: function (_ev: string, cb: any) {
          if (_ev === 'load' && typeof cb === 'function') {
            /* no-op */
          }
        },
      }),
...
describe('AnalyticsService (core)', () => {
  ...
  it('trackError emits app_error event', () => {
    ...
    const svc = new AnalyticsService({
      ...
      advanced: {
        ...baseConfig.advanced,
        onDispatch: e => events.push(e.event),
      },
    });
...
describe('AnalyticsService (providers & queues)', () => {
  ...
  it('dispatches RudderStack track', () => {
    ...
    const svc = new AnalyticsService({
      ...
      rudderstack: {
        enabled: true,
        writeKey: 'RS_KEY',
        dataPlaneUrl: 'https://rs.dataplane',
      },
  ...
  it('identify dispatches to PostHog', () => {
    ...
    expect((window as any).posthog.identify).toHaveBeenCalledWith(
      'ph-user',
      expect.any(Object)
    );
  });
  ...
  it('identify dispatches to RudderStack', () => {
    ...
    expect((window as any).rudderanalytics.identify).toHaveBeenCalledWith(
      'rs-user',
      expect.any(Object)
    );
  });
  ...
  it('auto error listener captures uncaught errors when enabled', () => {
    ...
    expect(
      events.some(
        e => e.event === 'app_error' && e.properties.name === 'uncaught_error'
      )
    ).toBe(true);
