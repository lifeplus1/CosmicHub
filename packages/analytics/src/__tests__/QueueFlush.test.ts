...
global.document = { head: { appendChild: () => {} }, createElement: () => ({ set async(v:any){}, src:'', onload:()=>{}, onerror:()=>{} }), getElementsByTagName: () => [] };
// @ts-expect-error test shim
...
describe('Queue & consent behavior', () => {
  it('queues events and page views until consent then flushes', () => {
    ...
    const svc = new AnalyticsService({ ...consentConfig, advanced: { ...consentConfig.advanced, onDispatch: e => dispatched.push(e.event) } });
